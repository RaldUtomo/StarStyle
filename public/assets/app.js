...
function initBookingTimeInteractions() {
    ...
    const submitButton = document.querySelector(".booking-time-continue");
    ...
    const syncSubmitState = () => {
        ...
        submitButton.disabled = !selectedInput || !selectedInput.value;
    };
...
function initBookingSummaryInteractions() {
    ...
    const selectedTimeOption = () => timeOptions.find((option) => option.classList.contains("is-selected")) || null;
    ...
    const syncStaffOptionsForTime = () => {
        const activeTime = selectedTimeOption();
        const allowedIds = new Set(
            String(activeTime?.dataset.availableStaffIds || "")
                .split(",")
                .map((value) => value.trim())
                .filter(Boolean)
        );
        ...
        staffOptions.forEach((option) => {
            ...
            const isVisible = allowedIds.size === 0 || allowedIds.has(staffId);
        ...
        const targetOption = selectedVisible ? staffOptions.find((option) => option.classList.contains("is-selected") && !option.hidden) : firstVisible;
        if (!targetOption) {
            if (selectedStaffInput) {
                selectedStaffInput.value = "0";
            }
            if (staffLabel) {
        ...
        if (staffLabel) {
            staffLabel.textContent = targetOption.dataset.staffLabel || "Staff";
        }
        if (selectedStaffInput) {
            selectedStaffInput.value = targetOption.dataset.staffId || "0";
        }
    };
...
function initCalendarEventViewer() {
    ...
    const renderProductResults = () => {
        ...
        const matches = productCatalog
            .filter((item) => normalizeText(`${item.name || ""} ${item.variant || ""} ${item.brand || ""}`).includes(query))
            .slice(0, 8);

        productResults.hidden = matches.length === 0;
        productResults.innerHTML = matches.map((item) => `
            <button type="button" class="calendar-agenda-product-panel__result" data-agenda-product-id="${escapeHtml(String(item.id || ""))}">
                <div>
    ...
    const renderSelectedProducts = () => {
        ...
        productSelected.innerHTML = draftProducts.map((item) => `
            <div class="calendar-agenda-product-row" data-agenda-selected-product="${escapeHtml(item.id)}">
                <span class="calendar-agenda-product-row__icon"><i class="bi bi-bottle"></i></span>
                <strong class="calendar-agenda-product-row__name">${escapeHtml(item.name)}${item.variant ? ` - ${escapeHtml(item.variant)}` : ""}</strong>
                <div class="calendar-agenda-product-row__qty">
...
function initSalesTabs() {
    ...
    const fabConfig = {
        ...
        "cash-drawer": { visible: true, icon: "plus-lg", menu: false },
        "cash-flow": { visible: true, icon: "plus-lg", menu: false },
    };
    ...
    const isSalesServiceFinalStatus = (status) => normalizeServiceViewStatus(status) === "completed";
    const shouldHideSalesServiceMore = (status) => {
        const normalized = normalizeServiceViewStatus(status);
        return normalized === "started" || normalized === "completed";
    };
    ...
    const openSalesServiceView = (reference) => {
        ...
        if (salesServiceNotes) {
            salesServiceNotes.hidden = !first.notes;
            salesServiceNotes.textContent = first.notes ? `Catatan: ${first.notes}` : "";
        }
    ...
    const applyServicesFilters = () => {
        ...
        if (servicesTotal) {
            servicesTotal.textContent = String(visibleCount);
        }
    };
    const downloadServicesSummary = (format) => {
        const visibleRows = servicesRows.filter((row) => !row.hidden);
        const lines = [
    ...
    const applyInvoiceFilters = () => {
        ...
        if (invoicesTotal) {
            invoicesTotal.textContent = String(visibleCount);
        }
    };
    const downloadInvoicesSummary = (format) => {
        const visibleRows = invoiceRows.filter((row) => !row.hidden);
        const lines = [
    ...
    const applyVoucherFilters = () => {
        ...
        if (vouchersTotal) {
            vouchersTotal.textContent = String(visibleCount);
        }
    };
    const downloadVouchersSummary = (format) => {
        const visibleRows = vouchersRows.filter((row) => !row.hidden);
        const lines = [
    ...
    fab?.addEventListener("click", () => {
        if (activeSalesTab === "cash-drawer") {
            if (typeof bootstrap !== "undefined" && openRegisterModal) {
                bootstrap.Modal.getOrCreateInstance(openRegisterModal).show();
            }
            return;
        }
...
function initStaffDatePickers({ shell, weekRange, monthRange, weekPicker }) {
    ...
    try {
        bookingMap = JSON.parse(shell.dataset.staffBookings || "{}");
    } catch (_error) {
        bookingMap = {};
    }
    ...
    const getBookingPayload = (staffId, date) => {
        const bookings = bookingMap?.[String(staffId)]?.[String(date)] || [];
        return Array.isArray(bookings) ? bookings : [];
    };
    const escapeStaffHtml = (value) => String(value ?? "").replace(/[&<>"']/g, (char) => ({
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
        '"': "&quot;",
    ...
    const renderBookingItems = (staffId, date, compact = false) => {
        const bookings = getBookingPayload(staffId, date);
        if (!bookings.length) {
            return "";
        }
        if (compact) {
            return `<div class="staff-booking-dots">${bookings.map((booking) => `<span title="${escapeStaffHtml(booking.reference || "Booking")}"></span>`).join("")}</div>`;
        }
        return `
            <div class="staff-booking-list">
                ${bookings.map((booking) => `
                    <div class="staff-booking-item">
                        <strong>${escapeStaffHtml(booking.start || "")}${booking.end ? ` - ${escapeStaffHtml(booking.end)}` : ""}</strong>
                        <span>${escapeStaffHtml(booking.reference || "Booking")}</span>
                        <small>${escapeStaffHtml(booking.service || "Booking")}</small>
                    </div>
                `).join("")}
            </div>
        `;
    ...
    const renderShiftCellContent = (staffId, date, todayIso) => {
        ...
        const bookingItems = renderBookingItems(staffId, date);
        if (!payload) {
            const baseContent = date === todayIso
            ...
            return `${baseContent}${bookingItems}`;
        }
        const labels = payload.shifts.map((shift) => `<span>${shift.start} - ${shift.end}</span>`).join("");
        return `<div class="staff-work-selected-hours">${labels}</div>${bookingItems}`;
    };
        ...
        const renderWeekSchedule = (start) => {
            ...
            weekRows.forEach((row) => {
                ...
                row.innerHTML = days.map((day) => {
                    ...
                    const payload = getShiftPayload(staffId, date);
                    const hasBookings = getBookingPayload(staffId, date).length > 0;
                    return `
                        <div class="staff-schedule-cell${payload ? " is-filled" : ""}${hasBookings ? " has-bookings" : ""}" data-staff-work-cell data-staff-id="${attr(staffId)}" data-staff-name="${attr(staffName)}" data-date="${attr(date)}" data-day-index="${day.getDay()}" data-day-label="${attr(labelText)}"${renderShiftCellAttrs(staffId, date)}>
            ...
            getMonthRows().forEach((row) => {
                ...
                row.innerHTML = days
                    .map((day) => {
                        ...
                        const hasBookings = getBookingPayload(staffId, dateStr).length > 0;
                        return `<div class="staff-month-cell${payload || hasBookings ? " is-filled" : ""}${hasBookings ? " has-bookings" : ""}" data-staff-work-cell data-staff-id="${attr(staffId)}" data-staff-name="${attr(staffName)}" data-date="${attr(dateStr)}" data-day-index="${date.getDay()}" data-day-label="${attr(dayLabel)}"${renderShiftCellAttrs(staffId, dateStr)}>${payload ? "" : '<button class="staff-schedule-add" type="button" aria-label="Tambah shift"><i class="bi bi-plus-lg"></i></button>'}${renderBookingItems(staffId, dateStr, true)}</div>`;
                    })
                    .join("");
            });
        };
