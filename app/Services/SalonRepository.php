...
final class SalonRepository
{
    ...
    private function staffDataDbAvailable(): bool
    {
        return $this->canUsePdo() && $this->tableExists('staff');
    }
    ...
    private function pdo(): \PDO
    {
        ...
        try {
            ...
            EmbeddedMariaDb::bootIfNeeded($this->config);
            $this->pdo = new \PDO($dsn, $username, $password, $options);
        }
    ...
    public function accountState(int $userId): array
    {
        ...
        $sessionState = $_SESSION['starstyle']['account'][$userId] ?? [];
        $staff = isset($user['staff_id']) ? $this->findStaff((int) $user['staff_id']) : null;
        ...
        if ($this->usingDb()) {
            ...
            $dbUser = $this->dbOne("SELECT {$select} FROM users WHERE id = :id LIMIT 1", ['id' => $userId]);
            if ($dbUser !== null) {
                $user = array_merge($user, $dbUser);
            }
        }

        $phone = (string) ($staff['phone'] ?? $sessionState['phone'] ?? '');
        $photoDataUrl = (string) ($staff['photo_data_url'] ?? $sessionState['photo_data_url'] ?? '');
        $avatar = (string) ($user['avatar'] ?? '');
        if ($avatar === '') {
            $avatar = strtoupper(substr(preg_replace('/\s+/', '', (string) ($user['name'] ?? 'Akun')), 0, 2));
        }

        return [
            'id' => $userId,
            'name' => (string) ($sessionState['name'] ?? $user['name'] ?? ''),
            'email' => (string) ($sessionState['email'] ?? $user['email'] ?? ''),
            'phone' => $phone,
            'role' => (string) ($staff['role_title'] ?? $staff['role'] ?? $user['role'] ?? 'Basic'),
            'avatar' => $avatar,
            'photo_data_url' => $photoDataUrl,
            'joined_year' => (string) ($sessionState['joined_year'] ?? '2026'),
            'email_verified' => !empty($user['email_verified_at']) || !empty($sessionState['email_verified']),
            'phone_verified' => !empty($user['phone_verified_at']) || !empty($sessionState['phone_verified']),
            'account_verified' => !empty($user['account_verified_at']) || !empty($sessionState['account_verified']),
            'otp_email_enabled' => (bool) ($user['otp_email_enabled'] ?? $sessionState['otp_email_enabled'] ?? false),
            'google_authenticator_enabled' => (bool) ($user['google_authenticator_enabled'] ?? $sessionState['google_authenticator_enabled'] ?? false),
            'facebook_connected' => (bool) ($user['facebook_connected'] ?? $sessionState['facebook_connected'] ?? false),
        ];
    }

    public function updateAccountProfile(int $userId, array $payload, string $actorName): array
    {
        ...
        $photoDataUrl = trim((string) ($payload['photo_data_url'] ?? ''));
        ...
        if ($phone !== '' && !preg_match('/^[+\d][\d\s-]{5,20}$/', $phone)) {
            throw new \InvalidArgumentException('Nomor ponsel tidak valid.');
        }
        if ($photoDataUrl !== '' && !preg_match('/^data:image\/(png|jpe?g|webp|gif);base64,/i', $photoDataUrl)) {
            throw new \InvalidArgumentException('Format foto tidak valid.');
        }

        $emailChanged = strcasecmp($email, (string) ($current['email'] ?? '')) !== 0;
        $avatar = strtoupper(substr(preg_replace('/\s+/', '', $name), 0, 2));
        $avatar = $avatar !== '' ? $avatar : 'AK';

        if ($this->usingDb()) {
            $this->ensureAccountSchema();
            $duplicate = $this->dbOne(
                "SELECT id FROM users WHERE LOWER(email) = LOWER(:email) AND id <> :id LIMIT 1",
                ['email' => $email, 'id' => $userId]
            );
            if ($duplicate !== null) {
                throw new \InvalidArgumentException('Email sudah digunakan akun lain.');
            }
            ...
            try {
                $this->dbExecute(
                    "UPDATE users
                     ...
                     WHERE id = :id",
                    ['id' => $userId, 'name' => $name, 'email' => $email, 'avatar' => $avatar, 'email_changed' => $emailChanged ? 1 : 0]
                );

                if (!empty($current['staff_id'])) {
                    $staffUpdates = [
                        'name = :name',
                        'email = :email',
                    ];
                    ...
                    if ($phone !== '') {
                        $staffUpdates[] = 'phone = :phone';
                        $params['phone'] = $phone;
                    }
                    if ($photoDataUrl !== '' && $this->columnExists('staff', 'photo_data_url')) {
                        $staffUpdates[] = 'photo_data_url = :photo_data_url';
                        $params['photo_data_url'] = $photoDataUrl;
                    }
                    $this->dbExecute(
                        'UPDATE staff SET ' . implode(', ', $staffUpdates) . ' WHERE id = :staff_id AND deleted_at IS NULL',
                        $params
                    );
                }
                ...
                if ($this->tableExists('activity_logs')) {
                    $this->dbExecute(
                        ...
                        [
                            ...
                            'action_text' => 'Memperbarui My Account',
                        ]
                    );
                }
            ...
            $_SESSION['starstyle']['account'][$userId] = array_merge($_SESSION['starstyle']['account'][$userId] ?? [], [
                'name' => $name,
                'email' => $email,
                'phone' => $phone,
                'photo_data_url' => $photoDataUrl,
        ...
        return $this->accountState($userId);
    }

    public function changeAccountPassword(int $userId, string $currentPassword, string $newPassword): void
    {
        $user = $this->findUserById($userId);
        if ($user === null) {
            throw new \InvalidArgumentException('Akun tidak ditemukan.');
        ...
        if ($this->usingDb()) {
            $this->dbExecute(
                ...
                ['id' => $userId, 'password' => password_hash($newPassword, PASSWORD_DEFAULT)]
            );
        } else {
            $_SESSION['starstyle']['account'][$userId]['password_changed_at'] = date('Y-m-d H:i:s');
        }
    }

    public function updateAccountSecurity(int $userId, array $payload): array
    {
        ...
        $state = [
            'otp_email_enabled' => !empty($payload['otp_email_enabled']),
            'google_authenticator_enabled' => !empty($payload['google_authenticator_enabled']),
            'facebook_connected' => !empty($payload['facebook_connected']),
        ];
        ...
        if ($this->usingDb()) {
            ...
            $this->dbExecute(
                "UPDATE users
                 SET otp_email_enabled = :otp_email_enabled,
                     google_authenticator_enabled = :google_authenticator_enabled,
                     facebook_connected = :facebook_connected
                 WHERE id = :id",
                [
                    'id' => $userId,
                    'otp_email_enabled' => $state['otp_email_enabled'] ? 1 : 0,
                    'google_authenticator_enabled' => $state['google_authenticator_enabled'] ? 1 : 0,
                    'facebook_connected' => $state['facebook_connected'] ? 1 : 0,
                ]
            );
        } else {
            $_SESSION['starstyle']['account'][$userId] = array_merge($_SESSION['starstyle']['account'][$userId] ?? [], $state);
        }

        return $this->accountState($userId);
    }

    public function verifyAccountContact(int $userId, string $type): array
    {
        if ($this->findUserById($userId) === null) {
            throw new \InvalidArgumentException('Akun tidak ditemukan.');
        }
        ...
        if ($this->usingDb()) {
            ...
            $sets = match ($type) {
                ...
                default => 'account_verified_at = NOW(), email_verified_at = COALESCE(email_verified_at, NOW()), phone_verified_at = COALESCE(phone_verified_at, NOW())',
            };
            $this->dbExecute('UPDATE users SET ' . $sets . ' WHERE id = :id', ['id' => $userId]);
        } else {
            $updates = match ($type) {
                'email' => ['email_verified' => true, 'email_verified_at' => $now],
                'phone' => ['phone_verified' => true, 'phone_verified_at' => $now],
                default => ['account_verified' => true, 'email_verified' => true, 'phone_verified' => true, 'account_verified_at' => $now],
            };
            $_SESSION['starstyle']['account'][$userId] = array_merge($_SESSION['starstyle']['account'][$userId] ?? [], $updates);
        }
    ...
    public function getPermissionsForUser(array $user): array
    {
        if ($this->usingDb()) {
            ...
            $stmt->execute(['user_id' => (int) ($user['id'] ?? 0)]);
            $rolePermissions = array_map(static fn (array $row): string => (string) $row['permission_key'], $stmt->fetchAll());

            if ($rolePermissions !== []) {
                return $rolePermissions;
            }

            return $this->permissions['defaults']['staff'] ?? [];
        }
    ...
    public function getCustomers(): array
    {
        ...
        return array_merge($this->baseData['customers'], $_SESSION['starstyle']['customers'] ?? []);
    }
    ...
    public function getReviews(array $filters = []): array
    {
        if ($this->usingDb()) {
            ...
            if ($search !== '') {
                $searchColumns = [
                    'LOWER(c.name) LIKE ?',
                    'LOWER(c.email) LIKE ?',
                    'LOWER(r.feedback) LIKE ?',
                    'CAST(r.booking_id AS CHAR) LIKE ?',
                ];
                if ($joinUsers) {
                    $searchColumns[] = 'LOWER(u.email) LIKE ?';
                }
                $where[] = '(' . implode(' OR ', $searchColumns) . ')';
                $needle = '%' . strtolower($search) . '%';
                for ($index = 0; $index < count($searchColumns); $index++) {
                    $params[] = $needle;
                }
            }
            ...
            $totalRows = (int) ($this->dbOne("SELECT COUNT(*) AS total FROM reviews")['total'] ?? 0);
            if ($reviews !== [] || $totalRows > 0) {
                return $reviews;
            }
        }
        ...
        return array_values(array_filter($reviews, static function (array $review) use ($rating, $search, $startDate, $endDate): bool {
            $reviewDate = substr((string) ($review['date'] ?? ''), 0, 10);
            if ($rating >= 1 && $rating <= 5 && (int) ($review['rating'] ?? 0) !== $rating) {
                return false;
            }
    ...
    public function getSettings(): array
    {
        if ($this->usingDb()) {
            ...
            return [
                ...
                'timezone' => (string) ($row['timezone'] ?? ($this->config['timezone'] ?? 'Asia/Jakarta')),
                'hours_schedule' => $schedule,
            ];
        }

        $settings = $_SESSION['starstyle']['settings'] ?? $this->baseData['settings'];
        $settings['timezone'] = (string) ($settings['timezone'] ?? ($this->config['timezone'] ?? 'Asia/Jakarta'));
    ...
    public function getNotifications(array $filters = []): array
    {
        if ($this->usingDb()) {
            ...
            if ($customer !== '' && $customer !== 'Pelanggan') {
                if ($joinUsers) {
                    $where[] = "COALESCE(NULLIF(u.name, ''), 'Pelanggan') = ?";
                    $params[] = $customer;
                } else {
                    $where[] = '? = ?';
                    $params[] = $customer;
            ...
            if ($search !== '') {
                ...
                if ($joinUsers) {
                    $searchColumns[] = 'LOWER(u.name) LIKE ?';
                    $searchColumns[] = 'LOWER(u.email) LIKE ?';
                }
                $where[] = '(' . implode(' OR ', $searchColumns) . ')';
                $needle = '%' . strtolower($search) . '%';
            ...
            $totalRows = (int) ($this->dbOne("SELECT COUNT(*) AS total FROM notifications")['total'] ?? 0);
            if ($notifications !== [] || $totalRows > 0) {
                return $notifications;
            }
        }

        $notifications = $this->baseData['notifications'];
        $customer = trim((string) ($filters['customer'] ?? ''));
        $search = strtolower(trim((string) ($filters['search'] ?? '')));

        return array_values(array_filter($notifications, static function (array $notification) use ($customer, $search): bool {
            $name = (string) (($notification['customer'] ?? '') ?: 'Pelanggan');
            if ($customer !== '' && $customer !== 'Pelanggan' && $name !== $customer) {
                return false;
            }
            if ($search !== '') {
                $haystack = strtolower(trim(implode(' ', [
                    ...
                    (string) ($notification['type_label'] ?? $notification['type'] ?? ''),
    ...
    public function getAttendance(): array
    {
        ...
        return $this->sessionAttendanceRows();
    }
    ...
    public function getTransactions(): array
    {
        if ($this->usingDb()) {
            $transactionRows = $this->dbAll(
                "SELECT t.id, t.booking_id, t.customer_id, t.staff_id, t.reference, t.payment_method, t.status,
                        t.discount_amount, t.rounding_amount, t.paid_at, b.location_id, b.start_at AS booking_start_at
                 FROM transactions t
            ...
            return array_map(static function (array $transaction) use ($itemsByTransaction): array {
                return [
                    ...
                    'location_id' => $transaction['location_id'] !== null ? (int) $transaction['location_id'] : 0,
                    'date' => (string) $transaction['paid_at'],
                    'booking_start_at' => (string) ($transaction['booking_start_at'] ?? $transaction['paid_at']),
    ...
    public function updateSalesInvoiceStatus(string $invoiceNumber, string $status, string $actorName): array
    {
        ...
        if ($invoiceNumber === '') {
            throw new \InvalidArgumentException('Nomor faktur tidak valid.');
        }

        $reference = str_starts_with($invoiceNumber, 'INV-')
            ? 'TRX-' . substr($invoiceNumber, 4)
            : $invoiceNumber;

        if ($this->usingDb()) {
            $transaction = $this->tableExists('invoices')
                ? $this->dbOne(
                    "SELECT t.id, t.reference, COALESCE(i.invoice_number, CONCAT('INV-', SUBSTRING(t.reference, 5))) AS invoice_number
                     FROM transactions t
                     LEFT JOIN invoices i ON i.transaction_id = t.id
                     WHERE i.invoice_number = :invoice_number
                        OR CONCAT('INV-', SUBSTRING(t.reference, 5)) = :invoice_number
                        OR t.reference = :reference
                     LIMIT 1",
                    [
                ...
                : $this->dbOne(
                    "SELECT t.id, t.reference, CONCAT('INV-', SUBSTRING(t.reference, 5)) AS invoice_number
                     FROM transactions t
                     WHERE CONCAT('INV-', SUBSTRING(t.reference, 5)) = :invoice_number
                        OR t.reference = :reference
                     LIMIT 1",
                    [
                        'invoice_number' => $invoiceNumber,
            ...
            try {
                ...
                $resolvedInvoice = (string) (($transaction['invoice_number'] ?? '') ?: $invoiceNumber);

                $this->dbExecute(
                    'UPDATE transactions SET status = :status WHERE id = :id',
                    ['status' => $normalizedStatus, 'id' => $transactionId]
                );

                if ($this->tableExists('invoices')) {
                    $this->dbExecute(
                        'UPDATE invoices SET status = :status WHERE transaction_id = :transaction_id',
                        ['status' => $normalizedStatus, 'transaction_id' => $transactionId]
                    );
                }
                ...
                if ($this->tableExists('activity_logs')) {
                    $this->dbExecute(
                        ...
                        [
                            'user_id' => null,
                            'actor_name' => $actorName,
                            'action_text' => 'Mengubah status faktur ' . $resolvedInvoice . ' menjadi ' . $statusLabel,
                        ]
                    );
                }
                ...
                return [
                    'message' => 'Status faktur berhasil diperbarui.',
                    'invoice' => [
        ...
        foreach ($_SESSION['starstyle']['transactions'] as $index => $transaction) {
            $transactionReference = (string) ($transaction['reference'] ?? '');
            $transactionInvoice = str_starts_with($transactionReference, 'TRX-')
                ? 'INV-' . substr($transactionReference, 4)
                : $transactionReference;

            if (!in_array($invoiceNumber, [$transactionInvoice, $transactionReference], true)
                && $reference !== $transactionReference) {
                continue;
            }

            $_SESSION['starstyle']['transactions'][$index]['status'] = $normalizedStatus;
            $_SESSION['starstyle']['activity_logs'][] = [
                'id' => $this->nextSessionId($_SESSION['starstyle']['activity_logs'] ?? []),
                'time' => date('Y-m-d H:i:s'),
                'actor' => $actorName,
                'action' => 'Mengubah status faktur ' . $transactionInvoice . ' menjadi ' . $statusLabel,
            ];
    ...
    public function saveSalesInvoicePaymentDetail(string $invoiceNumber, string $staffName, string $actorName): array
    {
        $invoiceNumber = trim($invoiceNumber);
        $staffName = trim($staffName) !== '' ? trim($staffName) : $actorName;
            ...
            if ($this->tableExists('activity_logs')) {
                $this->dbExecute(
                    ...
                    [
                        'user_id' => null,
                        'actor_name' => $actorName,
                        'action_text' => 'Menyimpan rincian pembayaran faktur ' . $invoiceNumber . ' oleh ' . $staffName,
                    ]
                );
            }

            return ['message' => 'Rincian pembayaran berhasil disimpan.'];
        }
        ...
        $_SESSION['starstyle']['activity_logs'][] = [
            ...
            'time' => date('Y-m-d H:i:s'),
            'actor' => $actorName,
            'action' => 'Menyimpan rincian pembayaran faktur ' . $invoiceNumber . ' oleh ' . $staffName,
        ];
    ...
    public function availability(array $serviceIds, int $staffId, string $date): array
    {
        return array_map(
            ...
            $this->bookingTimeAvailability($serviceIds, $date, $staffId)['slots'] ?? []
        );
    }

    public function bookingTimeAvailability(array $serviceIds, string $date, int $staffId = 0): array
    {
        $serviceIds = array_values(array_filter(array_map('intval', $serviceIds), static fn (int $serviceId): bool => $serviceId > 0));
        $settings = $this->getSettings();
        $timezone = $this->bookingTimezone();
        $targetDate = \DateTimeImmutable::createFromFormat('!Y-m-d', $date, $timezone) ?: new \DateTimeImmutable('today', $timezone);
        $businessHours = $this->businessHoursForDate($targetDate);
        $totalDuration = array_reduce($serviceIds, function (int $carry, int $serviceId): int {
            $service = $this->findService($serviceId);

            return $carry + max(0, (int) ($service['duration'] ?? 0));
        }, 0);
        $candidateStaff = $this->staffCandidatesForServices($serviceIds, $staffId);
        $now = new \DateTimeImmutable('now', $timezone);

        if ($businessHours === null || empty($businessHours['enabled'])) {
            return [
                'timezone' => $timezone->getName(),
                'current_time' => $now->format('H:i'),
                'business_hours' => 'Tutup',
                'is_closed' => true,
                'slots' => [],
                'staff_ids' => array_values(array_map(static fn (array $staff): int => (int) ($staff['id'] ?? 0), $candidateStaff)),
            ];
        }

        [$openHour, $openMinute] = array_map('intval', explode(':', (string) $businessHours['open']));
        [$closeHour, $closeMinute] = array_map('intval', explode(':', (string) $businessHours['close']));
        $openAt = $targetDate->setTime($openHour, $openMinute);
        $closeAt = $targetDate->setTime($closeHour, $closeMinute);
        if ($closeAt <= $openAt) {
            $closeAt = $closeAt->modify('+1 day');
        }

        $effectiveStart = $openAt;
        if ($targetDate->format('Y-m-d') === $now->format('Y-m-d')) {
            $effectiveStart = $this->roundUpToQuarterHour($now > $openAt ? $now : $openAt);
        }

        $latestStart = $closeAt->modify('-' . max(15, $totalDuration) . ' minutes');
        $slots = [];

        if ($effectiveStart <= $latestStart) {
            for ($start = $openAt; $start <= $latestStart; $start = $start->modify('+15 minutes')) {
                if ($start < $effectiveStart) {
                    continue;
                }

                $end = $start->modify('+' . max(15, $totalDuration) . ' minutes');
                $availableStaffIds = [];

                foreach ($candidateStaff as $staff) {
                    $candidateStaffId = (int) ($staff['id'] ?? 0);
                    if ($candidateStaffId <= 0) {
                        continue;
                    }

                    if (!$this->staffShiftAllowsRange($candidateStaffId, $start, $end)) {
                        continue;
                    }

                    $bookings = array_filter(
                        $this->getBookings(),
                        static fn (array $booking): bool => (int) ($booking['staff_id'] ?? 0) === $candidateStaffId
                            && str_starts_with((string) ($booking['start_at'] ?? ''), $start->format('Y-m-d'))
                            && in_array((string) ($booking['status'] ?? ''), ['new', 'pending', 'confirmed', 'arrived', 'started'], true)
                    );
                    $blocks = array_filter(
                        $this->getBlocks(),
                        static fn (array $block): bool => (int) ($block['staff_id'] ?? 0) === $candidateStaffId
                            && str_starts_with((string) ($block['start_at'] ?? ''), $start->format('Y-m-d'))
                    );

                    if ($this->hasOverlap($start, $end, $bookings, $blocks)) {
                        continue;
                    }
        ...
        return [
            'timezone' => $timezone->getName(),
            'current_time' => $now->format('H:i'),
            'business_hours' => sprintf('%s - %s', $openAt->format('H:i'), $closeAt->format('H:i')),
            'is_closed' => false,
            'slots' => $slots,
            'staff_ids' => array_values(array_unique(array_merge([], ...array_map(
                static fn (array $slot): array => array_map('intval', (array) ($slot['available_staff_ids'] ?? [])),
                $slots
            )))),
        ];
    }
    ...
    public function createBooking(array $payload, string $source = 'customer'): array
    {
        ...
        $slotAvailability = $this->bookingTimeAvailability($serviceIds, $date, $staffId);
        $matchingSlot = null;

        foreach ($slotAvailability['slots'] ?? [] as $slot) {
        ...
        $isSameReferenceUpdate = $existingBooking !== null
            && (int) ($existingBooking['staff_id'] ?? 0) === $staffId
            && str_starts_with((string) ($existingBooking['start_at'] ?? ''), $date . ' ' . $time);

        if (
            $source === 'customer'
            &&
            !$isSameReferenceUpdate
            && (
                !is_array($matchingSlot)
                || empty($matchingSlot['available'])
                || !in_array($staffId, array_map('intval', (array) ($matchingSlot['available_staff_ids'] ?? [])), true)
            )
        ) {
            return ['success' => false, 'message' => 'Jam booking sudah tidak tersedia. Silakan pilih slot lain yang masih kosong.'];
        }
        ...
        if ($this->usingDb()) {
            ...
            $oldStatus = (string) ($existingBooking['status'] ?? '');
            $status = $requestedStatus !== '' ? $requestedStatus : (string) ($existingBooking['status'] ?? 'new');
            $productsJson = $existingBooking['products_json'] ?? '[]';
            $cancelReason = $existingBooking['cancel_reason'] ?? null;
            $storedPaymentMethod = $paymentMethod !== '' ? $paymentMethod : (string) ($existingBooking['payment_method'] ?? '');
            $storedPaymentProofPath = $paymentProofPath !== '' ? $paymentProofPath : (string) ($existingBooking['payment_proof_path'] ?? '');
            $storedPaymentReviewStatus = array_key_exists('payment_review_status', $payload)
                ? $paymentReviewStatus
            ...
            try {
                ...
                if ($existingBooking !== null && $oldStatus !== '' && $oldStatus !== $status) {
                    $this->dbExecute(
                        ...
                        [
                            ...
                            'new_status' => $status,
                            'note' => $source === 'customer' ? 'Dikonfirmasi dari flow pembayaran pelanggan' : null,
                        ]
                    );
                }
        ...
        $existingPaymentMethod = $paymentMethod !== ''
            ? $paymentMethod
            : (is_array($existingBooking) ? (string) ($existingBooking['payment_method'] ?? '') : '');
        $existingPaymentProofPath = $paymentProofPath !== ''
            ? $paymentProofPath
            : (is_array($existingBooking) ? (string) ($existingBooking['payment_proof_path'] ?? '') : '');
        $existingPaymentReviewStatus = array_key_exists('payment_review_status', $payload)
            ? $paymentReviewStatus
            : (is_array($existingBooking)
                ? $this->normalizeBookingPaymentReviewStatus((string) ($existingBooking['payment_review_status'] ?? $paymentReviewStatus))
                : $paymentReviewStatus);
    ...
    public function checkout(array $payload, array $actor): array
    {
        ...
        $linkedBooking = null;
        if ($this->usingDb() && ($bookingId > 0 || $bookingReference !== '')) {
            $where = $bookingId > 0 ? 'id = :booking_key' : 'reference = :booking_key';
            $linkedBooking = $this->dbOne(
                "SELECT id, customer_id, staff_id, reference, status FROM bookings WHERE {$where} LIMIT 1",
                ['booking_key' => $bookingId > 0 ? $bookingId : $bookingReference]
            );
            if (is_array($linkedBooking)) {
                $bookingId = (int) ($linkedBooking['id'] ?? 0);
                $customerId = $customerId > 0 ? $customerId : (int) ($linkedBooking['customer_id'] ?? 0);
                $staffId = $staffId > 0 ? $staffId : (int) ($linkedBooking['staff_id'] ?? 0);
            }
        }
        if (!$this->usingDb() && ($bookingId > 0 || $bookingReference !== '')) {
            $linkedBooking = array_values(array_filter(
                $_SESSION['starstyle']['bookings'] ?? [],
                static fn (array $booking): bool => $bookingId > 0
                    ? (int) ($booking['id'] ?? 0) === $bookingId
                    : (string) ($booking['reference'] ?? '') === $bookingReference
            ))[0] ?? null;
            if (is_array($linkedBooking)) {
                $bookingId = (int) ($linkedBooking['id'] ?? 0);
            ...
            try {
                $this->dbExecute(
                    "INSERT INTO transactions (booking_id, customer_id, staff_id, reference, payment_method, status, discount_amount, rounding_amount, paid_at)
                     VALUES (:booking_id, :customer_id, :staff_id, :reference, :payment_method, 'paid', :discount, 0, NOW())",
                    [
    ...
    public function saveStaff(?int $staffId, array $payload, string $actorName): array
    {
        ...
        if (!$this->staffDataDbAvailable()) {
            $staffRows = $this->sessionStaffRows();
            if ($staffId === null) {
                $staffId = $this->nextSessionId($staffRows);
            } elseif ($existing === null) {
                throw new \InvalidArgumentException('Staff tidak ditemukan.');
            }

            $serviceIds = $record['service_ids_raw'];
            $commissionRules = json_decode((string) ($record['commission_rules'] ?? '[]'), true);
            $staffRows = array_values(array_filter(
                $staffRows,
                static fn (array $staff): bool => (int) ($staff['id'] ?? 0) !== $staffId
            ));
            $staffRows[] = array_merge($existing ?? [], [
                'id' => $staffId,
                ...
                'specialties' => $this->serviceNamesForIds($serviceIds),
                'service_ids' => $serviceIds,
                'commission_type' => $record['commission_type'],
                'commission_value' => (float) $record['commission_value'],
                'rating' => (float) ($record['rating'] ?? $existing['rating'] ?? 4.8),
                'location_name' => $this->locationNameById($record['location_id']),
                'gender' => $record['gender'],
                'booking_enabled' => (bool) $record['booking_enabled'],
                'agenda_color' => $record['agenda_color'],
                'started_working_on' => $record['started_working_on'],
                'ended_working_on' => $record['ended_working_on'],
                'public_title' => $record['public_title'],
                'notes' => $record['notes'],
                'instagram_handle' => $record['instagram_handle'],
                'photo_data_url' => $record['photo_data_url'],
                'commission_rules' => is_array($commissionRules) ? $commissionRules : [],
                'attendance_pose' => $record['attendance_pose'],
                'attendance_uploaded_pose' => $record['attendance_uploaded_pose'],
                'permissions' => $this->staffPermissionDefaultsByRoleTitle($record['role_title'], (bool) $record['booking_enabled']),
            ]);
            usort($staffRows, static fn (array $a, array $b): int => (int) ($a['id'] ?? 0) <=> (int) ($b['id'] ?? 0));
            $_SESSION['starstyle']['staff'] = $staffRows;
            $this->appendSessionActivity($actorName, ($existing === null ? 'Menambah' : 'Memperbarui') . ' staff #' . $staffId);
        ...
        try {
            ...
            if ($userId === null && $record['email'] !== null) {
                $matchedUser = $this->dbOne(
                    "SELECT u.id,
                     ...
                     FROM users u
                     LEFT JOIN staff s ON s.user_id = u.id AND s.deleted_at IS NULL
                     WHERE LOWER(u.email) = LOWER(:email) AND u.portal = 'internal'
                     LIMIT 1",
                    ['email' => $record['email']]
                );
                if ($matchedUser !== null && (int) ($matchedUser['staff_id'] ?? 0) === 0) {
                    $userId = (int) $matchedUser['id'];
                }
            }
            if ($userId !== null) {
                $linkedUser = $this->dbOne(
                ...
                $shouldSyncLinkedUser = strtolower((string) ($linkedUser['role'] ?? '')) === 'staff';
            }

            $userEmail = $this->resolveStaffUserEmail($userId, $record['email'], $record['name']);
            ...
            $staffRecord = $record;
            unset($staffRecord['service_ids_raw']);
            ...
            if ($staffId === null) {
                ...
                $this->dbExecute(
                    ...
                    $staffRecord + ['user_id' => $userId, 'staff_id' => $staffId]
                );
            }
    ...
    public function deleteStaff(int $staffId, string $actorName): void
    {
        if (!$this->staffDataDbAvailable()) {
            $staffRows = $this->sessionStaffRows();
            $before = count($staffRows);
            $staffRows = array_values(array_filter(
                $staffRows,
                static fn (array $staff): bool => (int) ($staff['id'] ?? 0) !== $staffId
            ));
            if (count($staffRows) === $before) {
                throw new \InvalidArgumentException('Staff tidak ditemukan.');
            }
            $_SESSION['starstyle']['staff'] = $staffRows;
            $this->appendSessionActivity($actorName, 'Menghapus staff #' . $staffId);
    ...
    public function saveStaffShifts(int $staffId, string $startDate, string $repeatMode, string $repeatEnd, ?string $repeatEndDate, array $shifts, string $actorName): void
    {
        ...
        if (!$this->staffDataDbAvailable()) {
            $shiftRows = array_values(array_filter(
                $this->sessionShiftRows(),
                static fn (array $shift): bool => (int) ($shift['staff_id'] ?? 0) !== $staffId
                    || !in_array((string) ($shift['date'] ?? ''), $dates, true)
            ));
            $nextId = $this->nextSessionId($shiftRows);
            ...
            $_SESSION['starstyle']['shifts'] = $shiftRows;
            $this->appendSessionActivity($actorName, 'Mengatur shift staff #' . $staffId . ' mulai ' . $startDate);
    ...
    public function deleteStaffShifts(int $staffId, string $startDate, string $repeatMode, string $repeatEnd, ?string $repeatEndDate, string $actorName): void
    {
        if (!$this->staffDataDbAvailable()) {
            ...
            $_SESSION['starstyle']['shifts'] = array_values(array_filter(
                $this->sessionShiftRows(),
                static fn (array $shift): bool => (int) ($shift['staff_id'] ?? 0) !== $staffId
                    || !in_array((string) ($shift['date'] ?? ''), $dates, true)
            ));
            $this->appendSessionActivity($actorName, 'Menghapus shift staff #' . $staffId . ' mulai ' . $startDate);
    ...
    public function saveStaffAttendance(array $payload, string $actorName): void
    {
        ...
        if ($this->findStaff($staffId) === null) {
            throw new \InvalidArgumentException('Staff tidak ditemukan.');
        }
        ...
        if (!$this->usingDb()) {
            $attendanceRows = array_values(array_filter(
                $this->sessionAttendanceRows(),
                static fn (array $attendance): bool => (int) ($attendance['staff_id'] ?? 0) !== $staffId
                    || (string) ($attendance['date'] ?? '') !== $attendanceDate
            ));
            $attendanceRows[] = [
                'staff_id' => $staffId,
                'date' => $attendanceDate,
                'shift_start' => substr($record['shift_start'], 0, 5),
                'shift_end' => substr($record['shift_end'], 0, 5),
                'clock_in' => substr($record['clock_in'], 0, 5),
                'clock_out' => substr($record['clock_out'], 0, 5),
                'source' => $record['source'],
                'status' => $record['status'],
                'selfie_in_score' => $record['selfie_in_score'],
                'selfie_out_score' => $record['selfie_out_score'],
            ];
            $_SESSION['starstyle']['attendance'] = $attendanceRows;
            $this->appendSessionActivity($actorName, 'Mengatur attendance staff #' . $staffId . ' tanggal ' . $attendanceDate);
    ...
    public function updateStaffAttendanceProfile(int $staffId, bool $active, string $pose, string $uploadedPose, string $actorName): void
    {
        if (!$this->usingDb()) {
            ...
            foreach ($staffRows as &$staff) {
                if ((int) ($staff['id'] ?? 0) !== $staffId) {
                    continue;
                }
                $staff['status'] = $active ? 'Aktif' : 'Nonaktif';
                $staff['attendance_pose'] = $pose !== '' ? $pose : 'Right Tilt';
                $staff['attendance_uploaded_pose'] = $uploadedPose;
                $updated = true;
                break;
            }
            unset($staff);
            ...
            $_SESSION['starstyle']['staff'] = $staffRows;
            $this->appendSessionActivity($actorName, 'Mengubah profil attendance staff #' . $staffId);
    ...
    public function findOrCreateGoogleCustomer(array $profile): array
    {
        if (!$this->usingDb()) {
            throw new \RuntimeException('Login Google membutuhkan koneksi database.');
        }

        $this->ensureCustomerSchema();
        $this->ensureGoogleCustomerSchema();
        ...
        if ($googleId === '' || $email === '') {
            throw new \InvalidArgumentException('Profil Google tidak lengkap.');
        }

        if ($name === '') {
            $name = ucfirst(strtok($email, '@') ?: 'Customer');
        }
        ...
        try {
            $user = $this->dbOne(
                "SELECT id
                 FROM users
                 WHERE portal = 'customer' AND google_id = :google_id
                 LIMIT 1",
                ['google_id' => $googleId]
            );

            if ($user === null) {
                $conflictingInternal = $this->dbOne(
                    "SELECT id
                     FROM users
                     WHERE portal = 'internal' AND LOWER(email) = LOWER(:email)
                     LIMIT 1",
                    ['email' => $email]
                );
                if ($conflictingInternal !== null) {
                    throw new \RuntimeException('Email Google ini sudah dipakai akun admin/staff.');
                }

                $user = $this->dbOne(
                    "SELECT id
                     FROM users
                     WHERE portal = 'customer' AND LOWER(email) = LOWER(:email)
                     LIMIT 1",
                    ['email' => $email]
                );
            }
            ...
            if ($user !== null) {
                ...
                $this->dbExecute(
                    ...
                    [
                        ...
                        'email' => $email,
                        'google_id' => $googleId,
                        'google_avatar_url' => $avatarUrl !== '' ? $avatarUrl : null,
                        'avatar' => $this->initialsForName($name),
                    ]
                );
                $customer = $this->dbOne(
                    "SELECT id
                ...
                if ($customer === null) {
                    $customer = $this->dbOne(
                        "SELECT id
                         FROM customers
                         WHERE user_id IS NULL AND LOWER(email) = LOWER(:email) AND deleted_at IS NULL
                         LIMIT 1",
                        ['email' => $email]
                    );
                }
                ...
                if ($customer === null) {
                    $this->dbExecute(
                        "INSERT INTO customers (user_id, member_id, name, phone, email, tags, status)
                         VALUES (:user_id, :member_id, :name, NULL, :email, JSON_ARRAY('Google'), 'Aktif')",
                        [
                            'user_id' => $userId,
                            'member_id' => $this->generateCustomerMemberId(),
                            'name' => $name,
                            'email' => $email,
                        ]
                    );
                ...
                $this->dbExecute(
                    "INSERT INTO users (role_id, name, email, password, portal, google_id, google_avatar_url, google_linked_at, avatar, is_active)
                     VALUES (:role_id, :name, :email, :password, 'customer', :google_id, :google_avatar_url, NOW(), :avatar, 1)",
                    [
            ...
            if ($this->tableExists('activity_logs')) {
                $this->dbExecute(
                    ...
                    [
                        'user_id' => $userId,
                        'actor_name' => $name,
                        'action_text' => 'Login customer via Google',
                    ]
                );
            }
        ...
        return $this->findUserById($userId) ?? throw new \RuntimeException('Akun Google gagal dimuat ulang.');
    }
    ...
    public function updateBusinessProfile(array $payload, string $actorName): array
    {
        ...
        $timezone = trim((string) ($payload['timezone'] ?? '')) ?: ($this->config['timezone'] ?? 'Asia/Jakarta');
    ...
    {
        if (!$this->staffDataDbAvailable() || $this->staffScheduleSchemaEnsured) {
            return;
        }
    ...
    {
        if (!$this->staffDataDbAvailable() || $this->staffScheduleSeedEnsured) {
            return;
        }
        ...
        if ($shiftCount === 0) {
            ...
            foreach ($staffRows as $staffRow) {
                foreach ($weekdayAnchors as $weekdayAnchor) {
                    $shiftDate = (new \DateTimeImmutable($weekdayAnchor))->format('Y-m-d');
                    $this->dbExecute(
                        "INSERT INTO staff_shifts (staff_id, shift_date, start_time, end_time, repeat_mode)
                         VALUES (:staff_id, :shift_date, :start_time, :end_time, :repeat_mode)",
                        [
    ...
    private function bootSessionState(): void
    {
        $_SESSION['starstyle'] ??= [
            ...
            'staff_permissions' => [],
            'activity_logs' => [],
            'settings' => $this->baseData['settings'],
        ];
    }

    private function sessionStaffRows(): array
    {
        if (!isset($_SESSION['starstyle']['staff']) || !is_array($_SESSION['starstyle']['staff'])) {
            $_SESSION['starstyle']['staff'] = $this->baseData['staff'];
        }

        return $_SESSION['starstyle']['staff'];
    }

    private function sessionShiftRows(): array
    {
        if (!isset($_SESSION['starstyle']['shifts']) || !is_array($_SESSION['starstyle']['shifts'])) {
            $_SESSION['starstyle']['shifts'] = $this->baseData['shifts'];
        }

        return $_SESSION['starstyle']['shifts'];
    }

    private function sessionAttendanceRows(): array
    {
        if (!isset($_SESSION['starstyle']['attendance']) || !is_array($_SESSION['starstyle']['attendance'])) {
            $_SESSION['starstyle']['attendance'] = $this->baseData['attendance'];
        }

        return $_SESSION['starstyle']['attendance'];
    }

    private function nextSessionId(array $rows): int
    {
    ...
    private function serviceNamesForIds(array $serviceIds): array
    {
        $names = [];
        foreach ($this->getServices() as $service) {
            if (in_array((int) ($service['id'] ?? 0), $serviceIds, true)) {
    ...
    private function locationNameById(?int $locationId): string
    {
        foreach ($this->getLocations() as $location) {
            if ((int) ($location['id'] ?? 0) === (int) $locationId) {
                return (string) ($location['name'] ?? 'Star Salon');
            }
        }
    ...
    private function appendSessionActivity(string $actorName, string $actionText): void
    {
        $_SESSION['starstyle']['activity_logs'][] = [
            'id' => $this->nextSessionId($_SESSION['starstyle']['activity_logs'] ?? []),
            'actor_name' => $actorName,
    ...
    private function ensureAccountSchema(): void
    {
        if (!$this->usingDb() || !$this->tableExists('users')) {
            return;
        }

        $columns = [
            'email_verified_at' => 'ADD COLUMN email_verified_at DATETIME NULL DEFAULT NULL AFTER avatar',
            'phone_verified_at' => 'ADD COLUMN phone_verified_at DATETIME NULL DEFAULT NULL AFTER email_verified_at',
            'account_verified_at' => 'ADD COLUMN account_verified_at DATETIME NULL DEFAULT NULL AFTER phone_verified_at',
            'otp_email_enabled' => 'ADD COLUMN otp_email_enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER account_verified_at',
            'google_authenticator_enabled' => 'ADD COLUMN google_authenticator_enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER otp_email_enabled',
            'facebook_connected' => 'ADD COLUMN facebook_connected TINYINT(1) NOT NULL DEFAULT 0 AFTER google_authenticator_enabled',
        ];

        foreach ($columns as $column => $definition) {
            if (!$this->columnExists('users', $column)) {
                $this->dbExecute("ALTER TABLE users {$definition}");
                $this->columnExistsCache['users.' . $column] = true;
            }
        }
    }
    private function ensureBusinessSettingsSchema(): void
    {
        ...
        $columns = [
            'timezone' => "ADD COLUMN timezone VARCHAR(80) NOT NULL DEFAULT 'Asia/Jakarta' AFTER notification_channel",
        ...
        foreach ($columns as $column => $definition) {
            if (!$this->columnExists('business_settings', $column)) {
                $this->dbExecute("ALTER TABLE business_settings {$definition}");
                $this->columnExistsCache['business_settings.' . $column] = true;
            }
        }
    }

    private function defaultBusinessHoursSchedule(): array
    {
        return [
            ['key' => 'minggu', 'label' => 'Minggu', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'senin', 'label' => 'Senin', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'selasa', 'label' => 'Selasa', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'rabu', 'label' => 'Rabu', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'kamis', 'label' => 'Kamis', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'jumat', 'label' => 'Jumat', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
            ['key' => 'sabtu', 'label' => 'Sabtu', 'enabled' => true, 'open' => '09:00', 'close' => '20:00'],
        ];
    }

    private function normalizeBusinessHoursSchedule(mixed $schedule): array
    {
        $defaults = $this->defaultBusinessHoursSchedule();
        $byKey = [];
        ...
        return array_map(static function (array $default) use ($byKey): array {
            $key = $default['key'];
            $item = $byKey[$key] ?? [];
            $open = preg_match('/^\d{2}:\d{2}$/', (string) ($item['open'] ?? '')) === 1 ? (string) $item['open'] : $default['open'];
            $close = preg_match('/^\d{2}:\d{2}$/', (string) ($item['close'] ?? '')) === 1 ? (string) $item['close'] : $default['close'];

            return [
                'key' => $key,
                'label' => (string) ($item['label'] ?? $default['label']),
                'enabled' => array_key_exists('enabled', $item) ? (bool) $item['enabled'] : (bool) $default['enabled'],
                'open' => $open,
                'close' => $close,
            ];
        }, $defaults);
    ...
    private function bookingTimezone(): \DateTimeZone
    {
        $timezoneName = trim((string) ($this->getSettings()['timezone'] ?? ($this->config['timezone'] ?? 'Asia/Jakarta')));
    ...
    private function businessHoursForDate(\DateTimeImmutable $date): ?array
    {
        $weekdayMap = [
            'Sun' => 'minggu',
            'Mon' => 'senin',
        ...
        $targetKey = $weekdayMap[$date->format('D')] ?? '';
        if ($targetKey === '') {
            return null;
        }

        foreach ($this->getSettings()['hours_schedule'] ?? [] as $day) {
            if (!is_array($day) || (string) ($day['key'] ?? '') !== $targetKey) {
                continue;
            }

            return [
                'key' => $targetKey,
                'label' => (string) ($day['label'] ?? ucfirst($targetKey)),
    ...
    private function roundUpToQuarterHour(\DateTimeImmutable $dateTime): \DateTimeImmutable
    {
        $minutes = (int) $dateTime->format('i');
        $remainder = $minutes % 15;
        $rounded = $dateTime->setTime((int) $dateTime->format('H'), $minutes, 0);

        if ($remainder === 0 && (int) $dateTime->format('s') === 0) {
            return $rounded;
        }

        return $rounded->modify('+' . (15 - $remainder) . ' minutes');
    }

    private function staffCandidatesForServices(array $serviceIds, int $preferredStaffId = 0): array
    {
        $staff = array_values(array_filter($this->getStaff(), function (array $staffMember) use ($serviceIds, $preferredStaffId): bool {
            $staffId = (int) ($staffMember['id'] ?? 0);
            if ($staffId <= 0 || !($staffMember['booking_enabled'] ?? true)) {
                return false;
            }

            if ($preferredStaffId > 0 && $staffId !== $preferredStaffId) {
                return false;
            }

            return $this->staffSupportsSelectedServices($staffMember, $serviceIds);
        }));

        usort($staff, static function (array $left, array $right): int {
    ...
    private function staffSupportsSelectedServices(array $staff, array $serviceIds): bool
    {
        if ($serviceIds === []) {
            return true;
        }

        $staffServiceIds = array_values(array_filter(array_map('intval', (array) ($staff['service_ids'] ?? [])), static fn (int $serviceId): bool => $serviceId > 0));
        if ($staffServiceIds === []) {
            return true;
        }

        foreach ($serviceIds as $serviceId) {
            if (!in_array((int) $serviceId, $staffServiceIds, true)) {
                return false;
            }
        }
    ...
    private function staffShiftAllowsRange(int $staffId, \DateTimeImmutable $start, \DateTimeImmutable $end): bool
    {
        $targetDate = $start->format('Y-m-d');
        $weekday = $start->format('D');
        $allShifts = array_values(array_filter($this->getShifts(), static fn (array $shift): bool => (int) ($shift['staff_id'] ?? 0) === $staffId));

        if ($allShifts === []) {
            return true;
        }

        $matchingShifts = array_values(array_filter($allShifts, static function (array $shift) use ($targetDate, $weekday): bool {
            $shiftDate = (string) ($shift['date'] ?? '');
            $repeatMode = strtolower(trim((string) ($shift['repeat_mode'] ?? 'none')));

            if ($shiftDate === $targetDate) {
                return true;
            }

            if ($repeatMode !== 'weekly' || $shiftDate === '' || $shiftDate > $targetDate) {
                return false;
            }

            $shiftWeekday = \DateTimeImmutable::createFromFormat('!Y-m-d', $shiftDate);

            return $shiftWeekday instanceof \DateTimeImmutable && $shiftWeekday->format('D') === $weekday;
        }));
        ...
        foreach ($matchingShifts as $shift) {
            if (preg_match('/^\d{2}:\d{2}$/', (string) ($shift['start'] ?? '')) !== 1 || preg_match('/^\d{2}:\d{2}$/', (string) ($shift['end'] ?? '')) !== 1) {
                continue;
            }

            [$shiftStartHour, $shiftStartMinute] = array_map('intval', explode(':', (string) $shift['start']));
            [$shiftEndHour, $shiftEndMinute] = array_map('intval', explode(':', (string) $shift['end']));
            $shiftStart = $start->setTime($shiftStartHour, $shiftStartMinute);
            $shiftEnd = $start->setTime($shiftEndHour, $shiftEndMinute);
    ...
    private function ensureGoogleCustomerSchema(): void
    {
        if (!$this->usingDb() || !$this->tableExists('users')) {
            return;
        }
        ...
        $columns = [
            'google_id' => 'ADD COLUMN google_id VARCHAR(80) NULL AFTER portal',
            'google_avatar_url' => 'ADD COLUMN google_avatar_url TEXT NULL AFTER google_id',
            'google_linked_at' => 'ADD COLUMN google_linked_at DATETIME NULL AFTER google_avatar_url',
        ];
    ...
    private function resolveCustomer(string $name, string $phone, string $email = '', int $preferredCustomerId = 0, bool $allowExistingMatch = false): int
    {
        if ($preferredCustomerId > 0 && $this->findCustomer($preferredCustomerId) !== null) {
        ...
        if ($this->usingDb()) {
                    ...
                    "SELECT id
                     ...
                     WHERE deleted_at IS NULL
                       AND (
                            ...
                            OR (:email_match <> '' AND LOWER(email) = LOWER(:email_value))
                       )
                     ORDER BY id ASC
                     LIMIT 1",
                    [
            ...
            $this->dbExecute(
                "INSERT INTO customers (member_id, name, phone, email, status)
                 VALUES (:member_id, :name, :phone, :email, 'Aktif')",
                [
                    'member_id' => $memberId,
        ...
        if ($allowExistingMatch) {
            foreach ($this->getCustomers() as $customer) {
                if (strcasecmp($customer['name'], $name) === 0 || ($phone !== '' && $customer['phone'] === $phone) || ($email !== '' && strcasecmp((string) ($customer['email'] ?? ''), $email) === 0)) {
                    return (int) $customer['id'];
                }
            }
        }

        $customerId = $this->nextId(array_merge($this->baseData['customers'], $_SESSION['starstyle']['customers'] ?? []), 9000);
        $_SESSION['starstyle']['customers'][] = [
            'id' => $customerId,
            'name' => $name !== '' ? $name : 'Walk In Customer',
            'gender' => '',
            'phone' => $phone,
            'email' => $email,
            'member_id' => 'MEM-' . str_pad((string) $customerId, 4, '0', STR_PAD_LEFT),
            'loyalty_points' => 0,
            'last_visit' => '',
            'birthdate' => '',
            'tags' => [],
    ...
    {
        $row = $this->dbOne("SELECT id FROM roles WHERE name = 'customer' LIMIT 1");

        return $row !== null ? (int) $row['id'] : 3;
    }

    private function initialsForName(string $name): string
    {
        $letters = strtoupper(substr(preg_replace('/\s+/', '', $name), 0, 2));

        return $letters !== '' ? $letters : 'CU';
    }

    private function resolveStaffUserEmail(?int $currentUserId, ?string $desiredEmail, string $name): string
    {
        if ($desiredEmail !== null) {
            ...
            if ($conflict === null || (int) $conflict['id'] === (int) $currentUserId) {
                return $desiredEmail;
            }
        }

        if ($currentUserId !== null) {
            $currentUser = $this->dbOne(
                "SELECT email
                 FROM users
                 WHERE id = :id
                 LIMIT 1",
        ...
        return $this->generateFallbackStaffEmail($name);
    }
    ...
    private function generateFallbackStaffEmail(string $name): string
    {
        ...
        do {
            $candidate = $slug . '.' . time() . '.' . random_int(100, 999) . '@starstyle.test';
            $existing = $this->dbOne(
                "SELECT id
                 FROM users
                 WHERE LOWER(email) = LOWER(:email)
    ...
    private function staffPermissionDefaultsByRoleTitle(string $roleTitle, bool $bookingEnabled): array
    {
        ...
        if (!$bookingEnabled) {
            $permissions = array_values(array_filter($permissions, static fn (string $permission): bool => $permission !== 'calendar.create'));
        }
