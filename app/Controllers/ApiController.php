...
final class ApiController extends BaseController
{
    ...
    public function staffShiftSave(): void
    {
        ...
        $this->json(['success' => true, 'shiftMap' => $this->staffShiftMapPayload()]);
    }
    ...
    private function staffShiftMapPayload(): array
    {
        $shiftMap = [];
        foreach ($this->repo()->getShifts() as $shift) {
            $date = (string) ($shift['date'] ?? $shift['shift_date'] ?? '');
            ...
            $shiftMap[(int) ($shift['staff_id'] ?? 0)][$date][] = [
    ...
    public function salesInvoiceStatus(): void
    {
        verify_csrf();
        $this->authorize('sales.view');

        try {
            $payload = $this->repo()->updateSalesInvoiceStatus(
    ...
    public function salesInvoicePaymentDetail(): void
    {
        verify_csrf();
        $this->authorize('sales.view');

        try {
            $payload = $this->repo()->saveSalesInvoicePaymentDetail(
                (string) ($_POST['invoice'] ?? ''),
    ...
    public function accountState(): void
    {
        $user = $this->internalUser();
        $this->json([
            'success' => true,
            'account' => $this->repo()->accountState((int) $user['id']),
        ]);
    }

    public function accountProfileSave(): void
    {
        verify_csrf();
        $user = $this->internalUser();

        try {
            $account = $this->repo()->updateAccountProfile((int) $user['id'], [
                'name' => (string) ($_POST['name'] ?? ''),
                'email' => (string) ($_POST['email'] ?? ''),
                'phone' => (string) ($_POST['phone'] ?? ''),
                'photo_data_url' => (string) ($_POST['photo_data_url'] ?? ''),
    ...
    public function accountPasswordSave(): void
    {
        verify_csrf();
        $user = $this->internalUser();

        try {
            $this->repo()->changeAccountPassword(
                (int) $user['id'],
        ...
        $this->json(['success' => true, 'message' => 'Password berhasil diperbarui.']);
    }

    public function accountSecuritySave(): void
    {
        verify_csrf();
        $user = $this->internalUser();

        try {
            $account = $this->repo()->updateAccountSecurity((int) $user['id'], [
                'otp_email_enabled' => (int) ($_POST['otp_email_enabled'] ?? 0) === 1,
                'google_authenticator_enabled' => (int) ($_POST['google_authenticator_enabled'] ?? 0) === 1,
                'facebook_connected' => (int) ($_POST['facebook_connected'] ?? 0) === 1,
            ]);
        } catch (\Throwable $throwable) {
            $this->json(['success' => false, 'message' => $throwable->getMessage()], 422);
        }
    ...
    public function accountVerify(): void
    {
        verify_csrf();
        $user = $this->internalUser();

        try {
            $account = $this->repo()->verifyAccountContact(
                (int) $user['id'],
                (string) ($_POST['type'] ?? 'profile')
            );
        } catch (\Throwable $throwable) {
            $this->json(['success' => false, 'message' => $throwable->getMessage()], 422);
        }
