...
$serviceReferenceImages = [
    'signature haircut' => 'https://rhairandbeauty.com.au/wp-content/uploads/2026/05/IMG-20241219-WA0030.jpg',
    'glossy balayage' => 'https://rhairandbeauty.com.au/wp-content/uploads/2026/05/Balayage-image-1.png',
    'keratin repair' => 'https://rhairandbeauty.com.au/wp-content/uploads/2026/05/Keratin-image.png',
    'relaxing head spa' => 'https://images.squarespace-cdn.com/content/v1/65a03ae547add4290dde7a6f/4c8a9ee5-2060-47aa-8858-5d17c7fc3554/0D6A9619.JPG',
];
$serviceImageResolver = static function (array $service) use ($coverImage, $serviceReferenceImages): string {
    $serviceImage = trim((string) ($service['image_data_url'] ?? ''));
    if ($serviceImage !== '') {
        return $serviceImage;
    }

    $name = strtolower(trim((string) ($service['name'] ?? '')));
    $description = strtolower(trim((string) ($service['description'] ?? '')));
    $haystack = $name . ' ' . $description;

    foreach ($serviceReferenceImages as $serviceKey => $serviceReferenceImage) {
        if (str_contains($haystack, $serviceKey)) {
            return $serviceReferenceImage;
        }
    }

    if (str_contains($haystack, 'keratin') || str_contains($haystack, 'repair') || str_contains($haystack, 'treatment') || str_contains($haystack, 'spa') || str_contains($haystack, 'head spa')) {
        return asset('img/treatment.png');
    }

    if (str_contains($haystack, 'color') || str_contains($haystack, 'balayage') || str_contains($haystack, 'glossy')) {
        return asset('img/ladies.png');
    }

    if (str_contains($haystack, 'haircut') || str_contains($haystack, 'cutting') || str_contains($haystack, 'styling')) {
        return asset('img/mens.png');
    }
    ...
    <div class="booking-picker-shell js-booking-picker">
        ...
        <div class="booking-picker-services js-booking-picker-services">
            <?php foreach ($services as $service): ?>
                <?php
                $serviceImage = $serviceImageResolver($service);
