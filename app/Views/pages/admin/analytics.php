...
$rangeStart = new DateTimeImmutable((string) ($analyticsFilters['start_date'] ?? $today->modify('first day of january this year')->format('Y-m-d')));
$rangeEnd = new DateTimeImmutable((string) ($analyticsFilters['end_date'] ?? $today->format('Y-m-d')));
$rangeLabel = (string) ($analyticsFilters['range_label'] ?? sprintf('Tahun ini, %s - %s', $rangeStart->format('j M Y'), $rangeEnd->format('j M Y')));
$activeRangePreset = (string) ($analyticsFilters['range'] ?? 'this_year');
$activeLocationId = (int) ($analyticsFilters['location_id'] ?? ($locationList[0]['id'] ?? 0));
$activeLocationLabel = (string) ($analyticsFilters['location_label'] ?? ($locationList[0]['name'] ?? 'Star Salon'));
$activeDateBasis = (string) ($analyticsFilters['date_basis'] ?? 'payment');
$activeAnalyticsTab = in_array((string) ($_GET['analytics_tab'] ?? 'overview'), ['overview', 'reports'], true)
    ? (string) ($_GET['analytics_tab'] ?? 'overview')
    : 'overview';
$activeReportItem = (string) ($_GET['report_item'] ?? 'finance-summary');
$analyticsQuery = static function (array $overrides = []) use ($activeLocationId, $activeRangePreset, $rangeStart, $rangeEnd, $activeDateBasis, $activeAnalyticsTab, $activeReportItem): string {
    $query = array_merge([
        'location_id' => $activeLocationId,
        'range' => $activeRangePreset,
        'start_date' => $rangeStart->format('Y-m-d'),
        'end_date' => $rangeEnd->format('Y-m-d'),
        'date_basis' => $activeDateBasis,
        'analytics_tab' => $activeAnalyticsTab,
        'report_item' => $activeReportItem,
    ], $overrides);

    return http_build_query($query);
};
$analyticsUrl = static fn (array $overrides = []): string => url('/analytics') . '?' . $analyticsQuery($overrides);
$analyticsExportUrl = static fn (array $overrides = []): string => url('/analytics/export') . '?' . $analyticsQuery(array_merge(['format' => 'csv'], $overrides));
...
<section class="analytics-shell js-analytics-shell" data-active-preset="<?= e($activeRangePreset) ?>" data-active-tab="<?= e($activeAnalyticsTab) ?>" data-active-report-item="<?= e($activeReportItem) ?>">
                ...
                <div class="analytics-toolbar__group">
                    <div class="dropdown">
                        <button class="dashboard-filter dashboard-filter--shop" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <span data-analytics-shop-label><?= e($activeLocationLabel) ?></span>
                            <i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="dropdown-menu analytics-filter-menu">
                            <?php foreach ($locationList as $location): ?>
                                <?php $locationId = (int) ($location['id'] ?? 0); ?>
                                <button class="dropdown-item analytics-filter-option <?= $locationId === $activeLocationId ? 'is-active' : '' ?>" type="button" data-analytics-shop-option="<?= e((string) ($location['name'] ?? '')) ?>" data-analytics-filter-url="<?= e($analyticsUrl(['location_id' => $locationId, 'analytics_tab' => 'overview'])) ?>">
                                    <?= e((string) ($location['name'] ?? 'Star Salon')) ?>
                                </button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <div class="analytics-detail-toolbar">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu analytics-filter-menu">
                                <?php foreach ($locationList as $location): ?>
                                    <?php $locationId = (int) ($location['id'] ?? 0); ?>
                                    <button class="dropdown-item analytics-filter-option <?= $locationId === $activeLocationId ? 'is-active' : '' ?>" type="button" data-analytics-shop-option="<?= e((string) ($location['name'] ?? '')) ?>" data-analytics-filter-url="<?= e($analyticsUrl(['location_id' => $locationId, 'analytics_tab' => 'reports'])) ?>">
                                        <?= e((string) ($location['name'] ?? 'Star Salon')) ?>
                                    </button>
                                <?php endforeach; ?>
                            </div>
                        ...
                        <a class="analytics-switch <?= $activeDateBasis === 'payment' ? 'is-active' : '' ?>" href="<?= e($analyticsUrl(['date_basis' => $activeDateBasis === 'payment' ? 'booking' : 'payment', 'analytics_tab' => 'reports'])) ?>" aria-pressed="<?= $activeDateBasis === 'payment' ? 'true' : 'false' ?>">
                            <span class="analytics-switch__track"></span>
                            <span>Berdasarkan tanggal pembayaran</span>
                        </a>
                        <div class="dropdown">
                            <button class="dashboard-filter" type="button" data-bs-toggle="dropdown" aria-expanded="false"><span>Export</span><i class="bi bi-caret-down-fill"></i></button>
                            <div class="dropdown-menu analytics-filter-menu">
                                <a class="dropdown-item analytics-filter-option" href="<?= e($analyticsExportUrl(['analytics_tab' => 'reports'])) ?>">Export CSV</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="customers-date-modal__body">
                <div class="customers-date-grid">
                    <div class="customers-date-presets">
                        <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'today' ? 'is-active' : '' ?>" type="button" data-preset="today">Hari ini</button>
                        <div class="customers-date-presets__row">
                            <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'this_month' ? 'is-active' : '' ?>" type="button" data-preset="this_month">Bulan ini</button>
                            <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'yesterday' ? 'is-active' : '' ?>" type="button" data-preset="yesterday">Kemarin</button>
                        </div>
                        <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === '7d' ? 'is-active' : '' ?>" type="button" data-preset="7d">7 hari sebelumnya</button>
                        <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === '30d' ? 'is-active' : '' ?>" type="button" data-preset="30d">30 hari sebelumnya</button>
                        <div class="customers-date-presets__row">
                            <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'last_month' ? 'is-active' : '' ?>" type="button" data-preset="last_month">Bulan kemarin</button>
                            <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'last_year' ? 'is-active' : '' ?>" type="button" data-preset="last_year">Tahun kemarin</button>
                        </div>
                        <button class="customers-date-preset js-analytics-date-preset <?= $activeRangePreset === 'this_year' ? 'is-active' : '' ?>" type="button" data-preset="this_year">Tahun ini</button>
                    </div>
                    ...
                    <div class="customers-date-picker">
                        <div class="customers-date-fields">
                            ...
                            <div>
                                ...
                                <input class="form-control customers-date-input js-analytics-end" type="text" value="<?= e($rangeEnd->format('Y-m-d')) ?>" placeholder="YYYY-MM-DD" autocomplete="off">
                            </div>
                        </div>
