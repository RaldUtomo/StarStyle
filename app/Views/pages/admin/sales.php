...
$serviceRangeEnd = $todayDate->modify('+30 days');
$serviceRangeLabel = sprintf(
    '30 hari sebelum/sesudah, %s - %s',
...
$salesRowTimestamp = static function (array $row, string $dateKey = 'date', string $timeKey = 'time'): int {
    $date = trim((string) ($row[$dateKey] ?? ''));
    $time = trim((string) ($row[$timeKey] ?? ''));
    if (str_contains($time, ' - ')) {
        $time = trim(explode(' - ', $time, 2)[0]);
    }
...
usort($serviceRows, static function (array $left, array $right) use ($salesRowTimestamp): int {
    return $salesRowTimestamp($right) <=> $salesRowTimestamp($left);
});
...
usort($invoiceRows, static function (array $left, array $right) use ($salesRowTimestamp): int {
    return $salesRowTimestamp($right) <=> $salesRowTimestamp($left);
});
...
usort($voucherRows, static function (array $left, array $right) use ($salesRowTimestamp): int {
    return $salesRowTimestamp($right, 'invoice_date', 'invoice_time') <=> $salesRowTimestamp($left, 'invoice_date', 'invoice_time');
});
...
<section class="sales-shell js-sales-shell"
         data-calendar-url="<?= e(url('/calendar?modal=agenda&entry=agenda')) ?>"
    ...
    <div class="sales-tab-panels">
        <section class="sales-panel is-active" data-sales-panel="daily">
            <div class="sales-toolbar">
                ...
                <div class="sales-toolbar__group sales-toolbar__group--end">
                    <label class="sales-switch js-sales-date-mode">
                        <input class="js-sales-date-mode-input" type="checkbox">
                        <span class="sales-switch__track"></span>
                        <span class="js-sales-date-mode-label">Berdasarkan tanggal invoice</span>
                    </label>
        ...
        <section class="sales-panel" data-sales-panel="services">
            ...
            <div class="sales-table-card sales-table-card--wide sales-table-card--services">
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-sales-services-pagination">
                    ...
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-sales-services-page-size-toggle" aria-expanded="false">
                            <span class="js-sales-services-page-size-label">20/page</span> <i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-sales-services-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-sales-services-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-sales-services-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-services-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-services-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-services-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-sales-services-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-services-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-sales-services-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-sales-services-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>
        </section>
            ...
            <div class="sales-table-card sales-table-card--wide sales-table-card--services sales-table-card--invoices">
                <div class="sales-table-scroll sales-table-scroll--services sales-table-scroll--invoices">
                    <table class="sales-table sales-table--wide sales-table--services sales-table--invoices">
                        <thead>
                            <tr>
                                <th class="sales-table__sticky-left">Faktur</th>
                                <th>Pelanggan</th>
                            ...
                            <?php else: ?>
                                <?php foreach ($invoiceRows as $row): ?>
                                    <tr class="js-sales-invoice-row"
                                        ...
                                        <td class="sales-table__sticky-left"><?= e($row['invoice']) ?></td>
                                        <td>
                                            <?php if (($row['customer_id'] ?? 0) > 0 && strcasecmp($row['customer'], 'Walk-In') !== 0): ?>
                                                <a
                                                    class="sales-customer-link js-sales-customer-link"
                                        ...
                                        <td class="sales-table__sticky-right"><button class="sales-status-pill js-sales-invoice-open <?= e($invoiceStatusClass($row['status'])) ?>" type="button"><?= e($row['status']) ?></button></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-sales-invoices-pagination">
                    <div class="sales-pagination__meta">Total <span class="js-sales-invoices-total"><?= e((string) count($invoiceRows)) ?></span></div>
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-sales-invoices-page-size-toggle" aria-expanded="false">
                            <span class="js-sales-invoices-page-size-label">20/page</span> <i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-sales-invoices-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-sales-invoices-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-sales-invoices-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-invoices-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-invoices-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-invoices-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-sales-invoices-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-invoices-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-sales-invoices-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-sales-invoices-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>
        </section>
        ...
        <section class="sales-panel" data-sales-panel="vouchers">
            ...
            <div class="sales-table-card sales-table-card--wide sales-table-card--services sales-table-card--vouchers">
                <div class="sales-table-scroll sales-table-scroll--services sales-table-scroll--vouchers">
                    <table class="sales-table sales-table--wide sales-table--services sales-table--vouchers">
                        <thead>
                            <tr>
                                <th class="sales-table__sticky-left">Nama Voucher</th>
                                <th>Kadaluarsa</th>
                                <th>Faktur</th>
                                <th>Pelanggan</th>
                                <th>Kode</th>
                            ...
                            <?php else: ?>
                                <?php foreach ($voucherRows as $row): ?>
                                    <tr class="js-sales-vouchers-row"
                                        ...
                                        <td class="sales-table__sticky-left"><?= e($row['name']) ?></td>
                                        <td><?= e($row['expired']) ?></td>
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-sales-vouchers-pagination">
                    <div class="sales-pagination__meta">Total <span class="js-sales-vouchers-total"><?= e((string) count($voucherRows)) ?></span></div>
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-sales-vouchers-page-size-toggle" aria-expanded="false">
                            <span class="js-sales-vouchers-page-size-label">20/page</span> <i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-sales-vouchers-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-sales-vouchers-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-sales-vouchers-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-vouchers-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-sales-vouchers-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-vouchers-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-sales-vouchers-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-sales-vouchers-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-sales-vouchers-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-sales-vouchers-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>
        </section>
    </div>
...
<section class="sales-invoice-view js-sales-invoice-view" hidden aria-label="Lihat faktur">
    ...
    <div class="sales-invoice-view__body">
        <div class="sales-invoice-view__left">
            <div class="sales-invoice-paper js-sales-invoice-paper">
            ...
            <div class="sales-invoice-a5 js-sales-invoice-a5" hidden>
                <div class="sales-invoice-a5__head">
                    <div class="sales-invoice-a5__logo"><i class="bi bi-shop"></i></div>
                    <div class="sales-invoice-a5__store">
                        <strong>Star Salon</strong>
                    ...
                    <div class="sales-invoice-a5__number">
                        <strong class="js-sales-invoice-a5-number">Faktur</strong>
                        <div class="js-sales-invoice-a5-date"></div>
                    </div>
                </div>
                <table class="sales-invoice-a5__table">
                    <thead>
                        <tr>
                            <th>Nama Item</th>
                            <th>QTY</th>
                    ...
                    <tbody class="js-sales-invoice-a5-items"></tbody>
                </table>
                <div class="sales-invoice-a5__totals">
                    <span>Sub Total</span><span class="js-sales-invoice-a5-subtotal">Rp 0,00</span>
                    <strong>Total</strong><span class="js-sales-invoice-a5-total">Rp 0,00</span>
                    <strong>Grand total</strong><span class="js-sales-invoice-a5-grand-total">Rp 0,00</span>
                    <strong class="js-sales-invoice-a5-payment-label" hidden>CASH</strong><span class="js-sales-invoice-a5-paid-total" hidden>Rp 0,00</span>
                    <strong>Sisa pembayaran</strong><span class="js-sales-invoice-a5-remaining">Rp 0,00</span>
                </div>
                <div class="sales-invoice-a5__seller">Penjualan oleh Rayhan Doni Pramana</div>
            </div>
            <div class="sales-invoice-floating-actions">
                <button type="button" class="sales-invoice-tool js-sales-invoice-format is-active" data-invoice-format="receipt" aria-label="Tampilan struk"><i class="bi bi-receipt-cutoff"></i></button>
                <button type="button" class="sales-invoice-tool js-sales-invoice-format" data-invoice-format="a5" aria-label="Tampilan A5">A5</button>
                <button type="button" class="sales-invoice-tool is-lower js-sales-invoice-download" aria-label="Download faktur"><i class="bi bi-download"></i></button>
                <button type="button" class="sales-invoice-tool js-sales-invoice-print" aria-label="Print faktur"><i class="bi bi-printer"></i></button>
            </div>
        </div>
        <aside class="sales-invoice-view__right">
            ...
            <div class="sales-invoice-actions">
                <div class="sales-invoice-more-wrap">
                    <button class="js-sales-invoice-more-toggle" type="button" aria-expanded="false">
                        Lainnya <i class="bi bi-caret-down-fill"></i>
                    </button>
                    <div class="sales-invoice-more-menu js-sales-invoice-more-menu" hidden>
                        <button class="js-sales-invoice-mark-unpaid" type="button">Ubah faktur unpaid</button>
                        <button class="js-sales-invoice-reschedule" type="button">Agendakan Ulang</button>
                        <button class="js-sales-invoice-void" type="button">Dibatalkan</button>
                        <button class="js-sales-invoice-payment-detail" type="button">Rincian Pembayaran</button>
                    </div>
                </div>
...
<div class="sales-invoice-payment-detail-modal js-sales-invoice-payment-detail-modal" hidden>
    <div class="sales-invoice-payment-detail">
        <div class="sales-invoice-payment-detail__header">
            <h2>Rincian Pembayaran</h2>
            <button class="js-sales-invoice-payment-detail-close" type="button" aria-label="Tutup">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        <div class="sales-invoice-payment-detail__body">
            <label for="salesInvoicePaymentStaff">Staf</label>
            <select id="salesInvoicePaymentStaff" class="js-sales-invoice-payment-staff">
                <?php foreach (($staff ?? []) as $staffMember): ?>
                    <option value="<?= e((string) ($staffMember['name'] ?? 'Staff')) ?>">
                        <?= e((string) ($staffMember['name'] ?? 'Staff')) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="sales-invoice-payment-detail__actions">
            <button class="js-sales-invoice-payment-detail-cancel" type="button">Batal</button>
            <button class="js-sales-invoice-payment-detail-save" type="button">Simpan</button>
        </div>
    </div>
</div>
            ...
            <div class="customers-date-modal__body">
                <div class="customers-date-grid">
                    <div class="customers-date-presets">
                        ...
                        <button class="customers-date-preset js-sales-services-date-preset is-active" type="button" data-preset="around_30d">30 hari sebelum/sesudah</button>
                        <button class="customers-date-preset js-sales-services-date-preset" type="button" data-preset="30d">30 hari sebelumnya</button>
                        <div class="customers-date-presets__row">
                            <button class="customers-date-preset js-sales-services-date-preset" type="button" data-preset="last_month">Bulan kemarin</button>
...
<div class="modal fade" id="salesCashDrawerDateFilterModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content customers-date-modal">
            <div class="customers-date-modal__header">
                <h2>Date Filter</h2>
            ...
            <div class="customers-date-modal__body">
                <div class="customers-date-grid">
                    <div class="customers-date-presets">
                        <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="today">Hari ini</button>
                        <div class="customers-date-presets__row">
                            <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="this_month">Bulan ini</button>
                            <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="yesterday">Kemarin</button>
                        </div>
                        <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="7d">7 hari sebelumnya</button>
                        <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="30d">30 hari sebelumnya</button>
                        <div class="customers-date-presets__row">
                            <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="last_month">Bulan kemarin</button>
                            <button class="customers-date-preset js-sales-cashdrawer-date-preset" type="button" data-preset="last_year">Tahun kemarin</button>
                        </div>
                        <button class="customers-date-preset js-sales-cashdrawer-date-preset is-active" type="button" data-preset="this_year">Tahun ini</button>
                    </div>
                    ...
                    <div class="customers-date-picker">
                        <div class="customers-date-fields">
                            <div>
                                <label>Mulai Tanggal</label>
                                <input class="form-control customers-date-input js-sales-cashdrawer-start" type="text" value="<?= e($cashDrawerRangeStart->format('Y-m-d')) ?>" placeholder="YYYY-MM-DD" autocomplete="off">
                            </div>
                            <div>
                                <label>Sampai Tanggal</label>
                                <input class="form-control customers-date-input js-sales-cashdrawer-end" type="text" value="<?= e($cashDrawerRangeEnd->format('Y-m-d')) ?>" placeholder="YYYY-MM-DD" autocomplete="off">
                            </div>
                        </div>

                        <div class="customers-date-inline">
                            <input class="js-sales-cashdrawer-date-range customers-date-range-input" type="text" aria-hidden="true" tabindex="-1">
                        </div>
                    </div>
                </div>
            </div>
            <div class="customers-date-modal__footer">
                <button type="button" class="customer-footer-btn js-sales-cashdrawer-date-reset">Reset</button>
                <button type="button" class="customer-footer-btn customers-date-apply js-sales-cashdrawer-date-apply" data-bs-dismiss="modal">Terapkan</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="salesOpenRegisterModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content sales-open-register-modal">
            <div class="sales-open-register-modal__header">
                <h2>Open Register</h2>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
            </div>
            <div class="sales-open-register-modal__body">
                <p>Untuk membuat penjualan silakan buka register terlebih dahulu</p>
                <div class="sales-open-register-modal__hint">Pastikan tidak ada kas register yang masih dalam status dibuka</div>
                <div class="sales-open-register-modal__field">
                    <label>Besaran Pembukaan Kas</label>
                    <input class="form-control js-sales-open-register-amount" type="text" value="Rp 0">
                </div>
                <div class="sales-open-register-modal__field">
                    <label>Catatan</label>
                    <textarea class="form-control js-sales-open-register-note" rows="3"></textarea>
                </div>
            </div>
            <div class="sales-open-register-modal__footer">
                <button type="button" class="sales-open-register-modal__cancel" data-bs-dismiss="modal">Tutup</button>
                <button type="button" class="sales-open-register-modal__save js-sales-open-register-save">Simpan</button>
            </div>
        </div>
    </div>
</div>

<div class="sales-register-view js-sales-register-view" hidden>
    <div class="sales-register-view__header">
        <div></div>
        <h2>Cash Register</h2>
        <button type="button" class="sales-register-view__close js-sales-register-close"><i class="bi bi-x-lg"></i></button>
    </div>
    <div class="sales-register-view__body">
        <div class="sales-register-view__summary">
            <div class="sales-register-view__identity">
                <div class="sales-register-view__avatar js-sales-register-avatar">R</div>
                <div class="sales-register-view__identity-copy">
                    <h3 class="js-sales-register-name">Rayhan Doni Pramana</h3>
                    <div class="sales-register-view__meta js-sales-register-meta"></div>
                    <p class="js-sales-register-note">-</p>
                </div>
                <span class="sales-status-pill js-sales-register-status">Tutup</span>
            </div>
            <div class="sales-register-view__totals">
                <strong>Total</strong>
                <div class="sales-register-view__totals-grid">
                    <div>
                        <span>Diharapkan</span>
                        <strong class="js-sales-register-expected">Rp 0,00</strong>
                    </div>
                    <div>
                        <span>Terhitung</span>
                        <strong class="js-sales-register-actual">Rp 0,00</strong>
                        <small class="js-sales-register-diff">Rp 0,00</small>
                    </div>
                </div>
            </div>
        </div>

        <div class="sales-register-view__toolbar">
            <div class="sales-register-view__tabs">
                <button class="is-active js-sales-register-tab" type="button" data-register-tab="transactions">Transaksi</button>
                <button class="js-sales-register-tab" type="button" data-register-tab="summary">Daftar Ringkasan</button>
            </div>
            <div class="sales-register-view__actions">
                <button type="button" class="js-sales-register-download"><i class="bi bi-download"></i> Unduh PDF</button>
                <button type="button" class="js-sales-register-email"><i class="bi bi-envelope"></i> Kirim daftar rangkuman</button>
            </div>
        </div>

        <div class="sales-register-view__panel is-active" data-register-panel="transactions">
            <table class="sales-table sales-table--wide">
                ...
                <tbody class="js-sales-register-transactions"></tbody>
            </table>
        </div>

        <div class="sales-register-view__panel" data-register-panel="summary">
            <table class="sales-table sales-table--wide">
                <thead>
                    <tr>
                        <th>Ringkasan</th>
                ...
                <tbody class="js-sales-register-summary"></tbody>
            </table>
        </div>
    </div>
    <button type="button" class="sales-register-view__floating js-sales-register-correction">Koreksi Register</button>
</div>
