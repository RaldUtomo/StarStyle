...
foreach ($bookings as $booking) {
    ...
    if (!in_array($status, ['confirmed', 'arrived', 'started'], true)) {
        continue;
    }
    ...
    $serviceNames = [];
    foreach ((array) ($booking['service_items'] ?? []) as $item) {
    ...
    $bookingMap[$staffId][$bookingDate][] = [
        'reference' => (string) ($booking['reference'] ?? '-'),
        'start' => substr((string) ($booking['start_at'] ?? ''), 11, 5),
        'end' => substr((string) ($booking['end_at'] ?? ''), 11, 5),
        'customer' => (string) ($booking['customer_name'] ?? 'Pelanggan'),
        'service' => $serviceNames !== [] ? implode(', ', array_unique($serviceNames)) : 'Booking',
        'status' => $status,
    ];
}

foreach ($bookingMap as $staffId => $dates) {
    foreach ($dates as $bookingDate => $rows) {
        usort($bookingMap[$staffId][$bookingDate], static fn (array $a, array $b): int => strcmp((string) ($a['start'] ?? ''), (string) ($b['start'] ?? '')));
    }
}

function render_staff_booking_items(array $items, bool $compact = false): string
{
    if ($items === []) {
        return '';
    }
    ...
    ob_start();
    ?>
    <div class="<?= $compact ? 'staff-booking-dots' : 'staff-booking-list' ?>">
        <?php foreach ($items as $item): ?>
            <?php if ($compact): ?>
                <span title="<?= e((string) ($item['reference'] ?? 'Booking')) ?>"></span>
            <?php else: ?>
                <div class="staff-booking-item">
                    <strong><?= e((string) ($item['start'] ?? '')) ?><?= !empty($item['end']) ? ' - ' . e((string) $item['end']) : '' ?></strong>
                    <span><?= e((string) ($item['reference'] ?? 'Booking')) ?></span>
                    <small><?= e((string) ($item['service'] ?? 'Booking')) ?></small>
                </div>
            <?php endif; ?>
        <?php endforeach; ?>
    </div>
...
<section class="staff-shell js-staff-shell"
         data-staff-shifts="<?= e(json_encode($shiftMap, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)) ?>"
         data-staff-bookings="<?= e(json_encode($bookingMap, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)) ?>">
    ...
    <div class="staff-panels">
        <section class="staff-panel is-active" data-staff-panel="work">
            ...
            <div class="staff-schedule-view is-active" data-staff-mode-panel="week">
                <div class="staff-schedule-card">
                                ...
                                <?php foreach ($featuredStaff as $member): ?>
                                    <div class="staff-schedule-days-grid staff-schedule-days-grid--week staff-week-days-row"
                                        ...
                                        <?php foreach ($weekDays as $day): ?>
                                            ...
                                            $hasShift = $dateKey === $today->format('Y-m-d');
                                            ?>
                    ...
                    <div class="staff-schedule-card__footer">
                        <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-work-pagination" data-staff-work-scope="week">
                            <div class="sales-pagination__meta">Total <span class="js-staff-work-total"><?= e((string) count($featuredStaff)) ?></span></div>
                            <div class="sales-pagination__select-wrap">
                                <button type="button" class="sales-pagination__select js-staff-work-page-size-toggle" aria-expanded="false">
                                    <span class="js-staff-work-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
                                </button>
                                <div class="sales-pagination__select-menu js-staff-work-page-size-menu" hidden>
                                    <button type="button" class="sales-pagination__select-option" data-staff-work-page-size="10">10/page</button>
                                    <button type="button" class="sales-pagination__select-option is-active" data-staff-work-page-size="20">20/page</button>
                                    <button type="button" class="sales-pagination__select-option" data-staff-work-page-size="30">30/page</button>
                                    <button type="button" class="sales-pagination__select-option" data-staff-work-page-size="50">50/page</button>
                                </div>
                            </div>
                            <button type="button" class="sales-pagination__nav js-staff-work-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                            <div class="sales-pagination__pages js-staff-work-pages">
                                <span class="sales-pagination__page is-active">1</span>
                            </div>
                            <button type="button" class="sales-pagination__nav js-staff-work-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                            <div class="sales-pagination__goto">Go to</div>
                            <input class="sales-pagination__input js-staff-work-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                            <button type="button" class="sales-pagination__top js-staff-work-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="staff-schedule-view" data-staff-mode-panel="month">
                <div class="staff-schedule-card">
                    ...
                    <div class="staff-schedule-card__footer">
                        <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-work-pagination" data-staff-work-scope="month">
                            <div class="sales-pagination__meta">Total <span class="js-staff-work-total"><?= e((string) count($featuredStaff)) ?></span></div>
                            <div class="sales-pagination__select-wrap">
                                <button type="button" class="sales-pagination__select js-staff-work-page-size-toggle" aria-expanded="false">
                                    <span class="js-staff-work-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
        ...
        <section class="staff-panel" data-staff-panel="members">
            ...
            <div class="sales-table-card sales-table-card--wide staff-member-table-card">
                <div class="sales-table-scroll staff-member-table-scroll">
                    <table class="sales-table sales-table--wide staff-member-table">
                        <thead>
                            <tr>
                                <th>Staf</th>
                                <th>Email</th>
                        ...
                        <tbody class="staff-member-list">
                        <?php if ($staff === []): ?>
                            <tr class="js-staff-members-empty">
                                <td colspan="4" class="sales-no-data">No Data</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($staff as $member): ?>
                                <?php $bookingEnabled = in_array('calendar.view', $member['permissions'] ?? ['calendar.view'], true); ?>
                                <tr class="staff-member-row"
                                    ...
                                    tabindex="0"
                                    role="button"
                                    aria-label="Edit staff <?= e($member['name']) ?>">
                                    <td class="staff-member-cell staff-member-cell--person">
                                        <button class="staff-member-edit-trigger" type="button" data-staff-edit-trigger aria-label="Edit staff <?= e($member['name']) ?>">
                                            <span class="staff-member-avatar">
                                                <?php if (!empty($member['photo_data_url'])): ?>
                                                    <img src="<?= e((string) $member['photo_data_url']) ?>" alt="<?= e($member['name']) ?>">
                                                <?php else: ?>
                                    ...
                                    <td class="staff-member-cell staff-member-cell--link"><?= e($member['email']) ?></td>
                                    <td class="staff-member-cell staff-member-cell--link"><?= e('+62' . preg_replace('/\D+/', '', $member['phone'])) ?></td>
                                    <td class="staff-member-cell">
                                        <span class="sales-status-pill staff-member-booking-pill <?= $bookingEnabled ? 'is-completed' : 'is-voided' ?>">
                                            <?= e($bookingEnabled ? 'Aktif' : 'Nonaktif') ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-members-pagination">
                <div class="sales-pagination__meta">Total <span class="js-staff-members-total"><?= e((string) count($staff)) ?></span></div>
                <div class="sales-pagination__select-wrap">
                    <button type="button" class="sales-pagination__select js-staff-members-page-size-toggle" aria-expanded="false">
                        <span class="js-staff-members-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
                    </button>
                    <div class="sales-pagination__select-menu js-staff-members-page-size-menu" hidden>
                        <button type="button" class="sales-pagination__select-option" data-staff-members-page-size="10">10/page</button>
                        <button type="button" class="sales-pagination__select-option is-active" data-staff-members-page-size="20">20/page</button>
                        <button type="button" class="sales-pagination__select-option" data-staff-members-page-size="30">30/page</button>
                        <button type="button" class="sales-pagination__select-option" data-staff-members-page-size="50">50/page</button>
                    </div>
                </div>
                <button type="button" class="sales-pagination__nav js-staff-members-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                <div class="sales-pagination__pages js-staff-members-pages">
                    <span class="sales-pagination__page is-active">1</span>
                </div>
                <button type="button" class="sales-pagination__nav js-staff-members-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                <div class="sales-pagination__goto">Go to</div>
                <input class="sales-pagination__input js-staff-members-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                <button type="button" class="sales-pagination__top js-staff-members-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
            </div>
        </section>
        ...
        <section class="staff-panel" data-staff-panel="attendance">
            ...
            <div class="sales-table-card sales-table-card--wide staff-attendance-staff-card" data-staff-attendance-view="staff">
                <div class="sales-table-scroll staff-attendance-staff-scroll">
                    <table class="staff-attendance-staff-table">
                        <thead>
                            <tr>
                                <th>Name</th>
                                <th>Face Photo</th>
                        ...
                        <tbody>
                            <?php foreach ($staff as $member): ?>
                                ...
                                <tr data-staff-attendance-row data-staff-id="<?= e((string) $member['id']) ?>" data-name="<?= e($member['name']) ?>" data-status="<?= e($member['status']) ?>" data-attendance-pose="<?= e((string) ($member['attendance_pose'] ?? 'Right Tilt')) ?>" data-attendance-uploaded-pose="<?= e((string) ($member['attendance_uploaded_pose'] ?? '')) ?>">
                                    ...
                                    <td data-attendance-last-modified><?= e($today->sub(new DateInterval('P30D'))->format('d M Y')) ?></td>
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-attendance-staff-pagination">
                    <div class="sales-pagination__meta">Total <span class="js-staff-attendance-staff-total"><?= e((string) count($staff)) ?></span></div>
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-staff-attendance-staff-page-size-toggle" aria-expanded="false">
                            <span class="js-staff-attendance-staff-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-staff-attendance-staff-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-staff-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-staff-attendance-staff-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-staff-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-staff-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-attendance-staff-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-staff-attendance-staff-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-attendance-staff-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-staff-attendance-staff-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-staff-attendance-staff-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>

            <div class="sales-table-card sales-table-card--wide sales-table-card--services staff-attendance-table-card" data-staff-attendance-view="attendance" hidden>
                <div class="sales-table-scroll sales-table-scroll--services staff-attendance-scroll">
                    <table class="sales-table sales-table--wide sales-table--services staff-attendance-table">
                        <thead>
                            <tr>
                                <th class="staff-attendance-date-col sales-table__sticky-left">Date</th>
                                <th>Staff</th>
                                <th>Shift</th>
                                <th>Clock In</th>
                                <th>Clock Out</th>
                        ...
                        <tbody>
                            <?php foreach ($staff as $member): ?>
                                ...
                                <tr data-staff-attendance-record data-staff-id="<?= e((string) $member['id']) ?>" data-name="<?= e($member['name']) ?>" data-status="<?= e($member['status']) ?>">
                                    <td class="staff-attendance-date-col sales-table__sticky-left"><?= e($today->format('Y-m-d')) ?></td>
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-attendance-record-pagination">
                    <div class="sales-pagination__meta">Total <span class="js-staff-attendance-record-total"><?= e((string) count($staff)) ?></span></div>
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-staff-attendance-record-page-size-toggle" aria-expanded="false">
                            <span class="js-staff-attendance-record-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-staff-attendance-record-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-record-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-staff-attendance-record-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-record-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-attendance-record-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-attendance-record-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-staff-attendance-record-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-attendance-record-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-staff-attendance-record-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-staff-attendance-record-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>
        ...
        <section class="staff-panel" data-staff-panel="commission">
            <div class="staff-toolbar staff-toolbar--commission sales-toolbar sales-toolbar--services">
                <div class="staff-toolbar__group sales-toolbar__group">
                    <div class="dropdown">
                ...
                <div class="staff-toolbar__group staff-toolbar__group--end sales-toolbar__group sales-toolbar__group--end">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu sales-toolbar-menu">
                            <button class="dropdown-item" type="button" data-staff-commission-export="pdf">PDF</button>
                            <button class="dropdown-item" type="button" data-staff-commission-export="xls">XLS</button>
                            <button class="dropdown-item" type="button" data-staff-commission-export="xlsx">XLSX</button>
                            <button class="dropdown-item" type="button" data-staff-commission-export="csv">CSV</button>
                        </div>
                    </div>
                    <div class="sales-search-combo">
                        <div class="dropdown">
                            <button class="dashboard-filter sales-search-select sales-toolbar-dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <span class="js-staff-commission-search-label">No Faktur</span><i class="bi bi-chevron-down"></i>
                            </button>
                            <div class="dropdown-menu sales-toolbar-menu">
                                <button class="dropdown-item is-active" type="button" data-staff-commission-search-option="invoice">No Faktur</button>
                                <button class="dropdown-item" type="button" data-staff-commission-search-option="staff">Anggota Staf</button>
                                <button class="dropdown-item" type="button" data-staff-commission-search-option="item">Barang Terjual</button>
                                <button class="dropdown-item" type="button" data-staff-commission-search-option="location">Lokasi</button>
                            </div>
                        </div>
                        <label class="sales-search-field sales-search-field--input staff-search-short staff-search-short--commission">
                            <input class="js-staff-commission-search" type="search" placeholder="Ketik kata kunci" autocomplete="off">
                            <i class="bi bi-search"></i>
                        </label>
                    </div>
                </div>
            ...
            <div class="sales-table-card sales-table-card--wide sales-table-card--services staff-commission-table">
                <div class="sales-table-scroll sales-table-scroll--services staff-commission-table__scroll">
                    <table class="sales-table sales-table--wide sales-table--services staff-commission-grid">
                        <thead>
                            <tr>
                                <th class="staff-commission-col-invoice sales-table__sticky-left">No Faktur</th>
                                ...
                                <th>Persen Komisi</th>
                                <th class="staff-commission-col-actions sales-table__sticky-right">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($commissionDummyEntries as $index => $entry): ?>
                                ...
                                <tr
                                    ...
                                    data-search-invoice="<?= e($entry['invoice_no']) ?>"
                                    data-search-staff="<?= e($entry['staff_name']) ?>"
                                    data-search-item="<?= e($entry['item_name']) ?>"
                                    data-search-location="<?= e($entry['location']) ?>"
                                >
                                    <td class="staff-commission-col-invoice sales-table__sticky-left">
                                    ...
                                    <td class="staff-commission-col-actions sales-table__sticky-right">
                                        <button class="sales-status-pill staff-commission-status <?= e($statusClass) ?>" type="button" data-commission-action="<?= e((string) $index) ?>" aria-label="Edit komisi <?= e($entry['staff_name']) ?>">
                                            <?= e(strtoupper((string) ($entry['status'] ?? 'draft'))) ?>
                                        </button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                ...
                <div class="sales-pagination sales-pagination--services sales-pagination--services-primary sales-pagination--fixed js-staff-commission-pagination">
                    ...
                    <div class="sales-pagination__select-wrap">
                        <button type="button" class="sales-pagination__select js-staff-commission-page-size-toggle" aria-expanded="false">
                            <span class="js-staff-commission-page-size-label">20/page</span><i class="bi bi-chevron-down"></i>
                        </button>
                        <div class="sales-pagination__select-menu js-staff-commission-page-size-menu" hidden>
                            <button type="button" class="sales-pagination__select-option" data-staff-commission-page-size="10">10/page</button>
                            <button type="button" class="sales-pagination__select-option is-active" data-staff-commission-page-size="20">20/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-commission-page-size="30">30/page</button>
                            <button type="button" class="sales-pagination__select-option" data-staff-commission-page-size="50">50/page</button>
                        </div>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-commission-prev" aria-label="Halaman sebelumnya"><i class="bi bi-chevron-left"></i></button>
                    <div class="sales-pagination__pages js-staff-commission-pages">
                        <span class="sales-pagination__page is-active">1</span>
                    </div>
                    <button type="button" class="sales-pagination__nav js-staff-commission-next" aria-label="Halaman berikutnya"><i class="bi bi-chevron-right"></i></button>
                    <div class="sales-pagination__goto">Go to</div>
                    <input class="sales-pagination__input js-staff-commission-page-current" type="text" value="1" inputmode="numeric" pattern="[0-9]*" aria-label="Go to page">
                    <button type="button" class="sales-pagination__top js-staff-commission-top" aria-label="Scroll ke atas"><i class="bi bi-chevron-up"></i></button>
                </div>
            </div>
        </section>
    </div>
