...
$currentUserStaffId = $currentUserRole === 'staff' ? (int) ($currentUser['staff_id'] ?? 0) : 0;
...
if ($requestedStaffId === null && $currentUserStaffId > 0) {
    foreach ($calendar['staff'] as $staffMember) {
        if ((int) $staffMember['id'] === $currentUserStaffId) {
            $activeStaff = $staffMember;
            break;
        }
    }
...
$calendarRenderStaffId = $requestedStaffId ? (int) $activeStaff['id'] : 0;
...
$calendarTransactions = app('repository')->getTransactions();
$paidBookingIds = [];
foreach ($calendarTransactions as $transaction) {
...
$calendarTimedItemsFor = static function (string $date, int $staffId) use (
    ...
    foreach ($calendarEvents as $event) {
        ...
        $eventBelongsToStaff = $showAllWorkingStaff || (int) $event['staff_id'] === $staffId;
        $hasMatchingStaff = $showAllWorkingStaff || $eventBelongsToStaff || array_filter(
            $eventServiceItems,
        ...
        if ($eventServiceItems !== []) {
            foreach ($eventServiceItems as $eventServiceItem) {
                ...
                if (!$showAllWorkingStaff && !$eventBelongsToStaff && $itemStaffId !== $staffId) {
                    continue;
                }
                ...
                $items[] = [
                    ...
                    'is_paid' => isset($paidBookingIds[(int) ($event['id'] ?? 0)]),
    ...
    foreach ($calendarBlocks as $block) {
        if (!$showBlocks || (!$showAllWorkingStaff && (int) $block['staff_id'] !== $staffId) || !str_starts_with($block['start_at'], $date)) {
            continue;
        }
...
<style>
    ...
    .calendar-agenda-product-panel[hidden] {
        display: none;
    }
    ...
    .calendar-agenda-branch-select {
        position: relative;
        display: inline-flex;
        --agenda-branch-pill-height: 58px;
    }
    ...
    .calendar-agenda-branch-menu button {
        width: 100%;
        min-height: var(--agenda-branch-pill-height);
        border: 0;
        border-radius: 0;
        background: #eef4ff;
        color: #111827;
    ...
    .calendar-agenda-invoice-actions .js-agenda-invoice-view-close {
        border-color: #dbe4f1;
        background: #fff;
        color: #5f82f2;
    }
    ...
    .calendar-agenda-branch-select {
        --agenda-branch-pill-height: 46px;
    }
    ...
    .calendar-agenda-branch-select {
        --agenda-branch-pill-height: 40px;
    }
                ...
                <div class="cal-body-row" style="height: <?= ($endHour + 1) * 120 ?>px;">
                    ...
                    <div class="cal-day-grid-wrap">
                        <div class="cal-day-grid-col cal-day-grid-col--single">
                            ...
                            <?php foreach ($calendarTimedItemsFor($calendar['date'], $calendarRenderStaffId) as $item): ?>
                                <?php
                                ...
                                <div class="calendar-event-blue calendar-event-card <?= e(trim($typeClass)) ?>"
                                     ...
                                     data-event-is-paid="<?= !empty($item['is_paid']) ? '1' : '0' ?>"
                    ...
                    <div class="cal-week-grid-col">
                        <?php foreach ($weekDays as $day): ?>
                            <div class="cal-day-grid-col">
                                ...
                                <?php foreach ($calendarTimedItemsFor($day['date'], $calendarRenderStaffId) as $item): ?>
                                    <?php
                                        $left = ((float) $item['column'] / (float) $item['columns']) * 100;
                                        $width = (1 / (float) $item['columns']) * 100;
                                        $statusKey = $calendarStatusKey((string) ($item['status'] ?? 'new'));
    ...
    <div class="modal-dialog modal-dialog-centered calendar-agenda-view-dialog">
        <div class="modal-content calendar-agenda-view">
            <div class="calendar-agenda-view__panel">
                ...
                <div class="calendar-agenda-view__content">
                ...
                <div class="calendar-agenda-view__products-summary js-agenda-view-products-summary">
                    <span class="calendar-agenda-view__products-icon"><i class="bi bi-bottle"></i></span>
                    <strong class="calendar-agenda-view__products-text js-agenda-view-products-count">0 Produk</strong>
                    <button type="button" class="calendar-agenda-view__products-edit js-agenda-view-products-edit" aria-label="Ubah produk">
                ...
                <div class="calendar-agenda-view__footer">
                    ...
                    <div class="calendar-agenda-view__more-wrap">
                        ...
                        <div class="calendar-agenda-view__more-menu js-agenda-view-more-menu" hidden>
                            <button type="button" class="calendar-agenda-view__more-option js-agenda-view-edit-trigger">
                                <i class="bi bi-pencil"></i>
                                <span>Ubah Agenda</span>
                            </button>
                            <button type="button" class="calendar-agenda-view__more-option js-agenda-view-add-product">
                                <i class="bi bi-plus-lg"></i>
                                <span>Tambahkan Produk</span>
                            </button>
                            <button type="button" class="calendar-agenda-view__more-option is-danger js-agenda-view-cancel-trigger">
                                <i class="bi bi-x-lg"></i>
                                <span>Batal</span>
                            </button>
                            <button type="button" class="calendar-agenda-view__more-option is-danger js-agenda-view-no-show-trigger">
                                <i class="bi bi-x-circle"></i>
                                <span>Tidak hadir</span>
                            </button>
                            <button type="button" class="calendar-agenda-view__more-option js-agenda-view-reset-no-show-trigger" hidden>
                                <i class="bi bi-arrow-repeat"></i>
            ...
            <aside class="calendar-agenda-product-panel js-agenda-product-panel" aria-hidden="true" hidden>
                <div class="calendar-agenda-product-panel__header">
                    <h3>Tambahkan Produk</h3>
                    <button type="button" class="calendar-agenda-product-panel__close js-agenda-product-close" aria-label="Tutup">
                        <i class="bi bi-x-lg"></i>
                    </button>
                </div>
                <div class="calendar-agenda-product-panel__search">
                    <input type="search" class="js-agenda-product-search" placeholder="Search Product..." autocomplete="off">
                </div>
                <div class="calendar-agenda-product-panel__results js-agenda-product-results" hidden></div>
                <div class="calendar-agenda-product-panel__empty js-agenda-product-empty">
                    <i class="bi bi-box-seam"></i>
                    <span>Belum ada produk untuk agenda ini</span>
                </div>
                <div class="calendar-agenda-product-panel__selected js-agenda-product-selected" hidden></div>
                <div class="calendar-agenda-product-panel__footer">
                    <button type="button" class="calendar-agenda-product-panel__cancel js-agenda-product-cancel">Batal</button>
                    <button type="button" class="calendar-agenda-product-panel__done js-agenda-product-done">Selesai</button>
                </div>
            </aside>
        </div>
    </div>
...
<div class="sales-service-toast js-calendar-agenda-toast" hidden>
    <div class="sales-service-toast__icon">
        <i class="bi bi-check-lg"></i>
    </div>
    <span>Saved</span>
    <button type="button" class="sales-service-toast__close js-calendar-agenda-toast-close" aria-label="Tutup">
        <i class="bi bi-x-lg"></i>
    </button>
</div>
                    ...
                    <div class="calendar-agenda-invoice-side js-agenda-invoice-side">
                        ...
                        <div class="calendar-agenda-invoice-info js-agenda-invoice-info-panel">
                            ...
                            <div class="calendar-agenda-invoice-actions js-agenda-invoice-actions">
                                ...
                                <button class="js-agenda-invoice-view-close" type="button">Tutup</button>
                            </div>
                        </div>
